...
/**
 * Testcase for canceling. A process execution is simulated in one thread. in another a cancel request by user is done. main thread (test) is blocked until cancel request goes through
	...
	public void process_execute_terminates_process_when_cancelation_is_done_by_cancel_state_provider()
		...
		SimpleProcessExecutor executorToTest = new SimpleProcessExecutor(null, false, 0){
			...
			boolean isAlive(Process p) {
				/* inconvenient workaround for process handling, but mockito often did not change the mocked result , so this way is necessary */
				return processAlive;
			}

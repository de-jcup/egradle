...
public class GradleOutputValidatorTest {
    ...
    public void test_bugfix_157_works() {
        ...
        assertEquals("C:\\dev_custom\\projects\\JCUP\\egradle-testcase-projects\\singleproject-01-java-with-gradlewrapper\\build.gradle", problem.getScriptPath());
    ...
    public void convert_problem_is_recognized() {
        output.add("* Where:");
        output.add("Build file '/home/albert/dev/src/git/egradle-testcase-projects/singleproject-01-java-with-gradlewrapper/build.gradle' line: 16");
        output.add("");
        output.add("* What went wrong:");
        ...
        output.add("");
        output.add("Comprehensive documentation on dependency notations is available in DSL reference for DependencyHandler type.");
        ...
        assertEquals("/home/albert/dev/src/git/egradle-testcase-projects/singleproject-01-java-with-gradlewrapper/build.gradle", problem.getScriptPath());
    ...
    public void compile_script_error() {
        ...
        output.add("> startup failed:");
        output.add("  script '/home/albert/dev/src/git/code2doc/libraries.gradle': 7: expecting ']', found 'mockito_all' @ line 7, column 3.");
     ...
     * <br>
     * This is not optimal and tests another component part, but it tests that each
     * output created by this tests, does not exceed the current shrink limit.
     * Otherwise the tests will not fail but it will not work in EGradle, because
     * the console line outputHandler shrinks the output to the limit given in
     * {@link DSLConstants#VALIDATION_OUTPUT_SHRINK_LIMIT}. So everything what is
     * tested here, has to work in EGradle too
     */
    protected void assertTestOutputDoesNotExceedLimits() {
        int size = output.size();
        assertTrue("validation has no problems, but not output contains too much lines - so limit constant must be made bigger, at least to:" + size, size <= Constants.VALIDATION_OUTPUT_SHRINK_LIMIT);
    }

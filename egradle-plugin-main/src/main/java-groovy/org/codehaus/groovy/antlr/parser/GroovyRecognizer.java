 ...
 *
 * [The -showtree option pops up a Swing frame that shows the AST constructed
 * from the parser.]
 *
 * Contributing authors: John Mitchell johnm@non.net Terence Parr
 * parrt@magelang.com John Lilley jlilley@empathy.com Scott Stanchfield
 * thetick@magelang.com Markus Mohnen mohnen@informatik.rwth-aachen.de Peter
 * Williams pete.williams@sun.com Allan Jacobs Allan.Jacobs@eng.sun.com Steve
 * Messick messick@redhills.com James Strachan jstrachan@protique.com John Pybus
 * john@pybus.org John Rose rose00@mac.com Jeremy Rayner groovy@ross-rayner.com
 * Alex Popescu the.mindstorm@gmail.com Martin Kempf mkempf@hsr.ch Reto Kleeb
 * rkleeb@hsr.ch
 *
 * Version 1.00 December 9, 1997 -- initial release Version 1.01 December 10,
 * 1997 fixed bug in octal def (0..7 not 0..8) Version 1.10 August 1998 (parrt)
 * added tree construction fixed definition of WS,comments for mac,pc,unix
 * newlines added unary plus Version 1.11 (Nov 20, 1998) Added "shutup" option
 * to turn off last ambig warning. Fixed inner class def to allow named class
 * defs as statements synchronized requires compound not simple statement add []
 * after builtInType DOT class in primaryExpression "const" is reserved but not
 * valid..removed from modifiers Version 1.12 (Feb 2, 1999) Changed LITERAL_xxx
 * to xxx in tree grammar. Updated java.g to use tokens {...} now for 2.6.0 (new
 * feature).
 *
 * Version 1.13 (Apr 23, 1999) Didn't have (stat)? for else clause in tree
 * parser. Didn't gen ASTs for interface extends. Updated tree parser too.
 * Updated to 2.6.0. Version 1.14 (Jun 20, 1999) Allowed final/abstract on local
 * classes. Removed local interfaces from methods Put instanceof precedence
 * where it belongs...in relationalExpr It also had expr not type as arg; fixed
 * it. Missing ! on SEMI in classBlock fixed: (expr) + "string" was parsed
 * incorrectly (+ as unary plus). fixed: didn't like Object[].class in parser or
 * tree parser Version 1.15 (Jun 26, 1999) Screwed up rule with instanceof in
 * it. :( Fixed. Tree parser didn't like (expr).something; fixed. Allowed
 * multiple inheritance in tree grammar. oops. Version 1.16 (August 22, 1999)
 * Extending an interface built a wacky tree: had extra EXTENDS. Tree grammar
 * didn't allow multiple superinterfaces. Tree grammar didn't allow empty var
 * initializer: {} Version 1.17 (October 12, 1999) ESC lexer rule allowed 399
 * max not 377 max. java.tree.g didn't handle the expression of synchronized
 * statements. Version 1.18 (August 12, 2001) Terence updated to Java 2 Version
 * 1.3 by observing/combining work of Allan Jacobs and Steve Messick. Handles
 * 1.3 src. Summary: o primary didn't include boolean.class kind of thing o
 * constructor calls parsed explicitly now: see explicitConstructorInvocation o
 * add strictfp modifier o missing objBlock after new expression in tree grammar
 * o merged local class definition alternatives, moved after declaration o fixed
 * problem with ClassName.super.field o reordered some alternatives to make
 * things more efficient o long and double constants were not differentiated
 * from int/float o whitespace rule was inefficient: matched only one char o add
 * an examples directory with some nasty 1.3 cases o made Main.java use buffered
 * IO and a Reader for Unicode support o supports UNICODE? Using Unicode
 * charVocabulary makes code file big, but only in the bitsets at the end. I
 * need to make ANTLR generate unicode bitsets more efficiently. Version 1.19
 * (April 25, 2002) Terence added in nice fixes by John Pybus concerning
 * floating constants and problems with super() calls. John did a nice reorg of
 * the primary/postfix expression stuff to read better and makes f.g.super()
 * parse properly (it was METHOD_CALL not a SUPER_CTOR_CALL). Also:
 *
 * o "finally" clause was a root...made it a child of "try" o Added stuff for
 * asserts too for Java 1.4, but *commented out* as it is not backward
 * compatible.
 *
 * Version 1.20 (October 27, 2002)
 *
 * Terence ended up reorging John Pybus' stuff to remove some nondeterminisms
 * and some syntactic predicates. Note that the grammar is stricter now; e.g.,
 * this(...) must be the first statement.
 *
 * Trinary ?: operator wasn't working as array name: (isBig ? bigDigits :
 * digits)[i];
 *
 * Checked parser/tree parser on source for Resin-2.0.5, jive-2.1.1, jdk 1.3.1,
 * Lucene, antlr 2.7.2a4, and the 110k-line jGuru server source.
 *
 * Version 1.21 (October 17, 2003) Fixed lots of problems including: Ray Waldin:
 * add typeDefinition to interfaceBlock in java.tree.g He found a problem/fix
 * with floating point that start with 0 Ray also fixed problem that (int.class)
 * was not recognized. Thorsten van Ellen noticed that \n are allowed
 * incorrectly in strings. TJP fixed CHAR_LITERAL analogously.
 *
 * Version 1.21.2 (March, 2003) Changes by Matt Quail to support generics (as
 * per JDK1.5/JSR14) Notes: o We only allow the "extends" keyword and not the
 * "implements" keyword, since that's what JSR14 seems to imply. o Thanks to
 * Monty Zukowski for his help on the antlr-interest mail list. o Thanks to Alan
 * Eliasen for testing the grammar over his Fink source base
 *
 * Version 1.22 (July, 2004) Changes by Michael Studman to support Java 1.5
 * language extensions Notes: o Added support for annotations types o Finished
 * off Matt Quail's generics enhancements to support bound type arguments o
 * Added support for new for statement syntax o Added support for static import
 * syntax o Added support for enum types o Tested against JDK 1.5 source base
 * and source base of jdigraph project o Thanks to Matt Quail for doing the hard
 * part by doing most of the generics work
 *
 * Version 1.22.1 (July 28, 2004) Bug/omission fixes for Java 1.5 language
 * support o Fixed tree structure bug with classOrInterface - thanks to Pieter
 * Vangorpto for spotting this o Fixed bug where incorrect handling of SR and
 * BSR tokens would cause type parameters to be recognised as type arguments. o
 * Enabled type parameters on constructors, annotations on enum constants and
 * package definitions o Fixed problems when parsing if ((char.class.equals(c)))
 * {} - solution by Matt Quail at Cenqua
 *
 * Version 1.22.2 (July 28, 2004) Slight refactoring of Java 1.5 language
 * support o Refactored for/"foreach" productions so that original literal "for"
 * literal is still used but the for sub-clauses vary by token type o Fixed bug
 * where type parameter was not included in generic constructor's branch of AST
 *
 * Version 1.22.3 (August 26, 2004) Bug fixes as identified by Michael Stahl;
 * clean up of tabs/spaces and other refactorings o Fixed typeParameters
 * omission in identPrimary and newStatement o Replaced GT reconcilliation code
 * with simple semantic predicate o Adapted enum/assert keyword checking support
 * from Michael Stahl's java15 grammar o Refactored typeDefinition production
 * and field productions to reduce duplication
 *
 * Version 1.22.4 (October 21, 2004) Small bux fixes o Added typeArguments to
 * explicitConstructorInvocation, e.g. new <String>MyParameterised() o Added
 * typeArguments to postfixExpression productions for anonymous inner class
 * super constructor invocation, e.g. new Outer().<String>super() o Fixed bug in
 * array declarations identified by Geoff Roy
 *
 * Version 1.22.4.g.1 o I have taken java.g for Java1.5 from Michael Studman
 * (1.22.4) and have applied the groovy.diff from java.g (1.22) by John Rose
 * back onto the new root (1.22.4) - Jeremy Rayner (Jan 2005)
 *
 * Version 1.22.4.g.2 o mkempf, rkleeb, Dec 2007 o fixed various rules so that
 * they call the correct Create Method to make sure that the line information
 * are correct
 *
...
public class GroovyRecognizer extends antlr.LLkParser implements GroovyTokenTypes {
	...
	/**
	 * This factory is the correct way to wire together a Groovy parser and
	 * lexer.
	 */
	public static GroovyRecognizer make(GroovyLexer lexer) {
		GroovyRecognizer parser = new GroovyRecognizer(lexer.plumb());
		// TODO: set up a common error-handling control block, to avoid
		// excessive tangle between these guys
		parser.lexer = lexer;
		lexer.parser = parser;
	...
	/**
	 * Create an AST node with the token type and text passed in, but with the
	 * same background information as another supplied Token (e.g.&nbsp;line
	 * numbers). To be used in place of antlr tree construction syntax, i.e.
	 * #[TOKEN,"text"] becomes create(TOKEN,"text",anotherToken)
	 *
	...
	public static boolean tracing = false; // only effective if antlr.Tool is
											// run with -traceParser
	...
	public void traceOut(String rname) throws TokenStreamException {
		if (!GroovyRecognizer.tracing)
			return;
		if (returnAST != null)
			rname += returnAST.toStringList();
		super.traceOut(rname);
	}

	// Error handling. This is a funnel through which parser errors go, when the
	// parser can suggest a solution.
	...
	// This is used to tell the difference (w/o symbol table access) between
	// {String x} and {println x}.
	private boolean isUpperCase(Token x) {
		if (x == null || x.getType() != IDENT)
			return false; // cannot happen?
		String xtext = x.getText();
		return (xtext.length() > 0 && Character.isUpperCase(xtext.charAt(0)));
	}

	private AST currentClass = null; // current enclosing class (for constructor
										// recognition)
	...
	/**
	 * Counts the number of LT seen in the typeArguments production. It is used
	 * in semantic predicates to ensure we have seen enough closing '>'
	 * characters; which actually may have been either GT, SR or BSR tokens.
	 */
	private int ltCounter = 0;

	/*
	 * This symbol is used to work around a known ANTLR limitation. In a loop
	 * with syntactic predicate, ANTLR needs help knowing that the loop exit is
	 * a second alternative. Example usage: ( (LCURLY)=> block |
	 * {ANTLR_LOOP_EXIT}? )* Probably should be an ANTLR RFE.
	 */
	...
	public final void annotationsOpt() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			annotationsOpt_AST = (AST) astFactory.make(
					(new ASTArray(2)).add(create(ANNOTATIONS, "ANNOTATIONS", first, LT(1))).add(annotationsOpt_AST));
	...
	public final void packageDefinition() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			packageDefinition_AST = (AST) astFactory
					.make((new ASTArray(3)).add(create(PACKAGE_DEF, "package", first, LT(1))).add(an_AST).add(id_AST));
	...
	/**
	 * A statement is an type of a block. Typical statements are declarations
	 * (which are scoped to the block) and expressions.
	 */
	public final void statement(int prevToken) throws RecognitionException, TokenStreamException {
		...
		case LITERAL_if: {
			...
			if (inputState.guessing == 0) {
				...
				statement_AST = (AST) astFactory.make((new ASTArray(4)).add(create(LITERAL_if, "if", first, LT(1)))
						.add(ale_AST).add(ifCbs_AST).add(elseCbs_AST));
		...
		case LITERAL_while: {
			...
			if (inputState.guessing == 0) {
				...
				if (s_AST != null)
					statement_AST = (AST) astFactory.make((new ASTArray(3))
							.add(create(LITERAL_while, "Literal_while", first, LT(1))).add(while_sce_AST).add(s_AST));
				else
					statement_AST = (AST) astFactory
							.make((new ASTArray(3)).add(create(LITERAL_while, "Literal_while", first, LT(1)))
									.add(while_sce_AST).add(while_cbs_AST));
		...
		case LITERAL_switch: {
			...
			nls();
			{
				_loop319: do {
					if ((LA(1) == LITERAL_default || LA(1) == LITERAL_case)) {
						...
						if (inputState.guessing == 0) {
							casesGroup_AST = (AST) astFactory
									.make((new ASTArray(3)).add(null).add(casesGroup_AST).add(cg_AST));
						}
					} else {
						break _loop319;
					}
			...
			if (inputState.guessing == 0) {
				statement_AST = (AST) currentAST.root;
				statement_AST = (AST) astFactory.make((new ASTArray(3))
						.add(create(LITERAL_switch, "switch", first, LT(1))).add(switchSce_AST).add(casesGroup_AST));
				currentAST.root = statement_AST;
				currentAST.child = statement_AST != null && statement_AST.getFirstChild() != null
						? statement_AST.getFirstChild() : statement_AST;
				currentAST.advanceChildToEnd();
						...
						if (synPredMatched306) {
							...
							if (synPredMatched317) {
								...
								if (inputState.guessing == 0) {
									statement_AST = (AST) currentAST.root;
									statement_AST = (AST) astFactory.make((new ASTArray(3))
											.add(create(LITERAL_synchronized, "synchronized", first, LT(1)))
											.add(synch_sce_AST).add(synch_cs_AST));
									currentAST.root = statement_AST;
									currentAST.child = statement_AST != null && statement_AST.getFirstChild() != null
											? statement_AST.getFirstChild() : statement_AST;
									currentAST.advanceChildToEnd();
	...
	/**
	 * A statement separator is either a semicolon or a significant newline. Any
	 * number of additional (insignificant) newlines may accompany it.
	 */
		...
		match(IDENT);
		{
			_loop72: do {
				if ((LA(1) == DOT)) {
					...
					match(IDENT);
					if (inputState.guessing == 0) {
						i1_AST = (AST) astFactory
								.make((new ASTArray(3)).add(create(DOT, ".", first, LT(1))).add(i1_AST).add(i2_AST));
					}
				} else {
					break _loop72;
				}
	...
	public final void importStatement() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (isStatic)
				importStatement_AST = (AST) astFactory.make((new ASTArray(3))
						.add(create(STATIC_IMPORT, "static_import", first, LT(1))).add(an_AST).add(is_AST));
			else
				importStatement_AST = (AST) astFactory
						.make((new ASTArray(3)).add(create(IMPORT, "import", first, LT(1))).add(an_AST).add(is_AST));
	...
	public final void identifierStar() throws RecognitionException, TokenStreamException {
		...
		}
		{
			...
			case DOT: {
				...
				if (inputState.guessing == 0) {
					i1_AST = (AST) astFactory
							.make((new ASTArray(3)).add(create(DOT, ".", first, LT(1))).add(i1_AST).add(s_AST));
				}
				break;
			}
			case LITERAL_as: {
				...
				match(IDENT);
				if (inputState.guessing == 0) {
					i1_AST = (AST) astFactory.make(
							(new ASTArray(3)).add(create(LITERAL_as, "as", first, LT(1))).add(i1_AST).add(alias_AST));
				}
				break;
			}
			case EOF:
	...
	protected final void typeDefinitionInternal(AST mods) throws RecognitionException, TokenStreamException {
		...
		case LITERAL_class: {
			...
			if (inputState.guessing == 0) {
				...
				currentAST.child = typeDefinitionInternal_AST != null
						&& typeDefinitionInternal_AST.getFirstChild() != null
	...
	public final void classDefinition(AST modifiers) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			classDefinition_AST = (AST) astFactory
					.make((new ASTArray(7)).add(create(CLASS_DEF, "CLASS_DEF", first, LT(1))).add(modifiers)
							.add(tmp29_AST).add(tp_AST).add(sc_AST).add(ic_AST).add(cb_AST));
	...
	public final void traitDefinition(AST modifiers) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			traitDefinition_AST = (AST) astFactory
					.make((new ASTArray(7)).add(create(TRAIT_DEF, "TRAIT_DEF", first, LT(1))).add(modifiers)
							.add(tmp31_AST).add(tp_AST).add(sc_AST).add(ic_AST).add(cb_AST));
	...
	public final void interfaceDefinition(AST modifiers) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			interfaceDefinition_AST = (AST) astFactory
					.make((new ASTArray(6)).add(create(INTERFACE_DEF, "INTERFACE_DEF", first, LT(1))).add(modifiers)
							.add(tmp33_AST).add(tp_AST).add(ie_AST).add(ib_AST));
	...
	public final void enumDefinition(AST modifiers) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			enumDefinition_AST = (AST) astFactory.make((new ASTArray(5)).add(create(ENUM_DEF, "ENUM_DEF", first, LT(1)))
					.add(modifiers).add(tmp35_AST).add(ic_AST).add(eb_AST));
	...
	public final void annotationDefinition(AST modifiers) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			annotationDefinition_AST = (AST) astFactory
					.make((new ASTArray(4)).add(create(ANNOTATION_DEF, "ANNOTATION_DEF", first, LT(1))).add(modifiers)
							.add(tmp38_AST).add(ab_AST));
	 ...
	 * or (if arguments are present) of a method. Generically, this is called a
	 * 'variable' definition, even in the case of a class field or method. It
	 * may start with the modifiers and/or a declaration keyword "def". It may
	 * also start with the modifiers and a capitalized type name.
	 * <p>
	...
	public final void modifiers() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			modifiers_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(MODIFIERS, "MODIFIERS", first, LT(1))).add(modifiers_AST));
	...
	/**
	 * The tail of a declaration. Either v1, v2, ... (with possible
	 * initializers) or else m(args){body}. The two arguments are the modifier
	 * list (if any) and the declaration head (if any). The declaration head is
	 * the variable type, or (for a method) the return type. If it is missing,
	 * then the variable type is taken from its initializer (if there is one).
	 * Otherwise, the variable type defaults to 'any'. DECIDE: Method return
	 * types default to the type of the method body, as an expression.
	 */
	public final void variableDefinitions(AST mods, AST t) throws RecognitionException, TokenStreamException {
		...
		if ((LA(1) == IDENT) && (_tokenSet_27.member(LA(2)))) {
			...
			if (inputState.guessing == 0) {
				...
				if (qid_AST != null)
					id_AST = qid_AST;
				variableDefinitions_AST = (AST) astFactory.make((new ASTArray(7))
						.add(create(METHOD_DEF, "METHOD", first, LT(1))).add(mods)
						.add((AST) astFactory.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t)))
						.add(id_AST).add(param_AST).add(tc_AST).add(mb_AST));
	...
	public final void typeParameters() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			typeParameters_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(TYPE_PARAMETERS, "TYPE_PARAMETERS", first, LT(1))).add(typeParameters_AST));
	...
	/**
	 * A declaration with one declarator and no initialization, like a
	 * parameterDeclaration. Used to parse loops like
	 * <code>for (int x in y)</code> (up to the <code>in</code> keyword).
	 */
	public final void singleDeclarationNoInit() throws RecognitionException, TokenStreamException {
		...
		case LITERAL_volatile: {
			...
			if (inputState.guessing == 0) {
				...
				currentAST.child = singleDeclarationNoInit_AST != null
						&& singleDeclarationNoInit_AST.getFirstChild() != null
	...
	/**
	 * Used in cases where a declaration cannot have commas, or ends with the
	...
	public final void singleVariable(AST mods, AST t) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			singleVariable_AST = (AST) astFactory.make((new ASTArray(4))
					.add(create(VARIABLE_DEF, "VARIABLE_DEF", first, LT(1))).add(mods)
					.add((AST) astFactory.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t)))
					.add(id_AST));
	...
	/**
	 * A declaration with one declarator and optional initialization, like a
	 * parameterDeclaration. Used to parse declarations used for both binding
	 * and effect, in places like argument lists and <code>while</code>
	 * statements.
	 */
	...
	/**
	 * Used only as a lookahead predicate, before diving in and parsing a
	 * declaration. A declaration can be unambiguously introduced with "def", an
	 * annotation or a modifier token like "final". It may also be introduced by
	 * a simple identifier whose first character is an uppercase letter, as in
	 * {String x}. A declaration can also be introduced with a built in type
	 * like 'int' or 'void'. Brackets (array and generic) are allowed, as in
	 * {List[] x} or {int[][] y}. Anything else is parsed as a statement of some
	 * sort (expression or command).
	 * <p>
	 * (In the absence of explicit method-call parens, we assume a capitalized
	 * name is a type name. Yes, this is a little hacky. Alternatives are to
	 * complicate the declaration or command syntaxes, or to have the parser
	 * query the symbol table. Parse-time queries are evil. And we want both
	 * {String x} and {println x}. So we need a syntactic razor-edge to slip
	...
	public final void annotation() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			annotation_AST = (AST) astFactory.make(
					(new ASTArray(3)).add(create(ANNOTATION, "ANNOTATION", first, LT(1))).add(i_AST).add(args_AST));
	...
	/**
	 * An IDENT token whose spelling is required to start with an uppercase
	 * letter. In the case of a simple statement {UpperID name} the identifier
	 * is taken to be a type name, not a command name.
	 */
	...
	public final void typeArguments() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			typeArguments_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(TYPE_ARGUMENTS, "TYPE_ARGUMENTS", first, LT(1))).add(typeArguments_AST));
	...
	/**
	 * lookahead predicate for usage of generics in methods as parameter for the
	 * method. Example: static <T> T foo(){} <T> must be first after the
	 * modifier. This rule allows more and does no exact match, but it is only a
	 * lookahead, not the real rule.
	 */
	...
	public final void modifiersOpt() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			modifiersOpt_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(MODIFIERS, "MODIFIERS", first, LT(1))).add(modifiersOpt_AST));
	...
	public final void classTypeSpec(boolean addImagNode) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (addImagNode) {
				classTypeSpec_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(classTypeSpec_AST));
			}
	...
	public final void builtInTypeSpec(boolean addImagNode) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (addImagNode) {
				builtInTypeSpec_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(builtInTypeSpec_AST));
			}
	...
	public final void classOrInterfaceType(boolean addImagNode) throws RecognitionException, TokenStreamException {
		...
		}
		{
			_loop47: do {
				if ((LA(1) == DOT) && (LA(2) == IDENT)) {
					...
					if (inputState.guessing == 0) {
						i1_AST = (AST) astFactory.make((new ASTArray(4)).add(create(DOT, ".", first, LT(1))).add(i1_AST)
								.add(i2_AST).add(ta_AST));
					}
				} else {
					break _loop47;
				}
		...
		if (inputState.guessing == 0) {
			...
			if (addImagNode) {
				classOrInterfaceType_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(classOrInterfaceType_AST));
			}
	...
	/**
	 * After some type names, where zero or more empty bracket pairs are
	 * allowed. We use ARRAY_DECLARATOR to represent this.
	 */
		...
		}
		{
			_loop260: do {
				if ((LA(1) == LBRACK) && (LA(2) == RBRACK)) {
					...
					if (inputState.guessing == 0) {
						declaratorBrackets_AST = (AST) currentAST.root;
						declaratorBrackets_AST = (AST) astFactory.make((new ASTArray(2))
								.add(create(ARRAY_DECLARATOR, "[", typ, LT(1))).add(declaratorBrackets_AST));
						currentAST.root = declaratorBrackets_AST;
						currentAST.child = declaratorBrackets_AST != null
								&& declaratorBrackets_AST.getFirstChild() != null
										? declaratorBrackets_AST.getFirstChild() : declaratorBrackets_AST;
						currentAST.advanceChildToEnd();
					}
				} else {
	...
	public final void typeArgumentsDiamond() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			typeArgumentsDiamond_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(TYPE_ARGUMENTS, "TYPE_ARGUMENTS", first, LT(1))).add(typeArgumentsDiamond_AST));
	...
	public final void builtInTypeArraySpec(boolean addImagNode) throws RecognitionException, TokenStreamException {
		...
		bt_AST = (AST) returnAST;
		{
			...
			if (synPredMatched66) {
				...
				if (inputState.guessing == 0) {
					require(false, "primitive type parameters not allowed here",
		...
		if (inputState.guessing == 0) {
			...
			if (addImagNode) {
				builtInTypeArraySpec_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(builtInTypeArraySpec_AST));
			}
	...
	public final void typeArgument() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			typeArgument_AST = (AST) astFactory.make(
					(new ASTArray(2)).add(create(TYPE_ARGUMENT, "TYPE_ARGUMENT", first, LT(1))).add(typeArgument_AST));
	...
	public final void typeArgumentBounds() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (isUpperBounds) {
				typeArgumentBounds_AST = (AST) astFactory.make((new ASTArray(2))
						.add(create(TYPE_UPPER_BOUNDS, "TYPE_UPPER_BOUNDS", first, LT(1))).add(typeArgumentBounds_AST));
			} else {
				typeArgumentBounds_AST = (AST) astFactory.make((new ASTArray(2))
						.add(create(TYPE_LOWER_BOUNDS, "TYPE_LOWER_BOUNDS", first, LT(1))).add(typeArgumentBounds_AST));
			}
	...
	public final void annotationArguments() throws RecognitionException, TokenStreamException {
		...
		if ((_tokenSet_47.member(LA(1))) && (_tokenSet_48.member(LA(2)))) {
			...
			if (inputState.guessing == 0) {
				...
				annotationArguments_AST = (AST) astFactory.make((new ASTArray(3))
						.add(create(ANNOTATION_MEMBER_VALUE_PAIR, "ANNOTATION_MEMBER_VALUE_PAIR", LT(1), LT(1))).add(i)
						.add(v_AST));
	...
	public final void annotationMemberValuePair() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			annotationMemberValuePair_AST = (AST) astFactory.make((new ASTArray(3))
					.add(create(ANNOTATION_MEMBER_VALUE_PAIR, "ANNOTATION_MEMBER_VALUE_PAIR", first, LT(1))).add(i_AST)
					.add(v_AST));
			currentAST.root = annotationMemberValuePair_AST;
			currentAST.child = annotationMemberValuePair_AST != null
					&& annotationMemberValuePair_AST.getFirstChild() != null
	...
	public final void conditionalExpression(int lc_stmt) throws RecognitionException, TokenStreamException {
		...
		astFactory.addASTChild(currentAST, returnAST);
		{
			...
			if (((LA(1) == ELVIS_OPERATOR || LA(1) == NLS) && (_tokenSet_50.member(LA(2))))) {
				int _m456 = mark();
			...
			if (synPredMatched456) {
				...
				if (((LA(1) == QUESTION || LA(1) == NLS) && (_tokenSet_51.member(LA(2))))) {
					int _m458 = mark();
	...
	public final void superClassClause() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			superClassClause_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(EXTENDS_CLAUSE, "EXTENDS_CLAUSE", first, LT(1))).add(c_AST));
	...
	public final void implementsClause() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			implementsClause_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(IMPLEMENTS_CLAUSE, "IMPLEMENTS_CLAUSE", first, LT(1))).add(implementsClause_AST));
	...
	public final void classBlock() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			classBlock_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(OBJBLOCK, "OBJBLOCK", first, LT(1))).add(classBlock_AST));
	...
	public final void interfaceExtends() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			interfaceExtends_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(EXTENDS_CLAUSE, "EXTENDS_CLAUSE", first, LT(1))).add(interfaceExtends_AST));
	...
	public final void interfaceBlock() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			interfaceBlock_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(OBJBLOCK, "OBJBLOCK", first, LT(1))).add(interfaceBlock_AST));
	...
	public final void enumBlock() throws RecognitionException, TokenStreamException {
		...
		nls();
		{
			...
			if (((LA(1) == IDENT || LA(1) == AT) && (_tokenSet_54.member(LA(2))))) {
				int _m137 = mark();
		...
		if (inputState.guessing == 0) {
			...
			enumBlock_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(OBJBLOCK, "OBJBLOCK", first, LT(1))).add(enumBlock_AST));
	...
	public final void annotationBlock() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			annotationBlock_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(OBJBLOCK, "OBJBLOCK", first, LT(1))).add(annotationBlock_AST));
	...
	public final void typeParameter() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			typeParameter_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(TYPE_PARAMETER, "TYPE_PARAMETER", first, LT(1))).add(typeParameter_AST));
	...
	public final void typeParameterBounds() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			typeParameterBounds_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(TYPE_UPPER_BOUNDS, "TYPE_UPPER_BOUNDS", first, LT(1))).add(typeParameterBounds_AST));
				...
				if (synPredMatched201) {
					...
					if (synPredMatched203) {
						...
						if (synPredMatched205) {
							...
							if (inputState.guessing == 0) {
								classField_AST = (AST) currentAST.root;
								classField_AST = (AST) astFactory.make((new ASTArray(2))
										.add(create(STATIC_INIT, "STATIC_INIT", first, LT(1))).add(s3_AST));
								currentAST.root = classField_AST;
								currentAST.child = classField_AST != null && classField_AST.getFirstChild() != null
										? classField_AST.getFirstChild() : classField_AST;
								currentAST.advanceChildToEnd();
							...
							if (inputState.guessing == 0) {
								classField_AST = (AST) currentAST.root;
								classField_AST = (AST) astFactory.make((new ASTArray(2))
										.add(create(INSTANCE_INIT, "INSTANCE_INIT", first, LT(1))).add(s4_AST));
								currentAST.root = classField_AST;
								currentAST.child = classField_AST != null && classField_AST.getFirstChild() != null
										? classField_AST.getFirstChild() : classField_AST;
								currentAST.advanceChildToEnd();
			...
			case LITERAL_double: {
				...
				t_AST = (AST) returnAST;
				{
					...
					if (synPredMatched161) {
						...
						if (inputState.guessing == 0) {
							annotationField_AST = (AST) currentAST.root;
							annotationField_AST = (AST) astFactory.make((new ASTArray(5))
									.add(create(ANNOTATION_FIELD_DEF, "ANNOTATION_FIELD_DEF", first, LT(1)))
									.add(mods_AST)
									.add((AST) astFactory
											.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t_AST)))
									.add(i_AST).add(amvi_AST));
							currentAST.root = annotationField_AST;
							currentAST.child = annotationField_AST != null
									&& annotationField_AST.getFirstChild() != null ? annotationField_AST.getFirstChild()
											: annotationField_AST;
							currentAST.advanceChildToEnd();
						}
	...
	public final void enumConstant() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			enumConstant_AST = (AST) astFactory
					.make((new ASTArray(5)).add(create(ENUM_CONSTANT_DEF, "ENUM_CONSTANT_DEF", first, LT(1)))
					...
					case SEMI: {
						{
											...
											case SEMI: {
												if (inputState.guessing == 0) {
													astFactory.addASTChild(currentAST,
															astFactory.create(EMPTY_STAT, "EMPTY_STAT"));
												}
												break;
											}
											default: {
							...
							if (inputState.guessing == 0) {
								...
								argList_AST = (AST) astFactory.make((new ASTArray(2))
										.add(create(CLOSURE_LIST, "CLOSURE_LIST", first, LT(1))).add(argList_AST));
					...
					case RPAREN: {
						{
											...
											case RPAREN: {
												{
													if (inputState.guessing == 0) {
														if (trailingComma)
															throw new NoViableAltException(lastComma, getFilename());
														trailingComma = true;
							...
							if (inputState.guessing == 0) {
								argList_AST = (AST) currentAST.root;
								argList_AST = (AST) astFactory.make(
										(new ASTArray(2)).add(create(ELIST, "ELIST", first, LT(1))).add(argList_AST));
								currentAST.root = argList_AST;
								currentAST.child = argList_AST != null && argList_AST.getFirstChild() != null
										? argList_AST.getFirstChild() : argList_AST;
								currentAST.advanceChildToEnd();
	...
	public final void enumConstantBlock() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			enumConstantBlock_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(OBJBLOCK, "OBJBLOCK", first, LT(1))).add(enumConstantBlock_AST));
	...
	public final void enumConstantField() throws RecognitionException, TokenStreamException {
				...
				if (synPredMatched174) {
					...
					if (inputState.guessing == 0) {
						...
						currentAST.child = enumConstantField_AST != null
								&& enumConstantField_AST.getFirstChild() != null ? enumConstantField_AST.getFirstChild()
										: enumConstantField_AST;
						currentAST.advanceChildToEnd();
					}
				} else {
					...
					if (synPredMatched176) {
						...
						if (inputState.guessing == 0) {
							...
							currentAST.root = enumConstantField_AST;
							currentAST.child = enumConstantField_AST != null
									&& enumConstantField_AST.getFirstChild() != null
											? enumConstantField_AST.getFirstChild() : enumConstantField_AST;
							currentAST.advanceChildToEnd();
						}
					} else if ((_tokenSet_68.member(LA(1))) && (_tokenSet_16.member(LA(2)))) {
						modifiersOpt();
		...
		case LCURLY: {
			...
			if (inputState.guessing == 0) {
				enumConstantField_AST = (AST) currentAST.root;
				enumConstantField_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(INSTANCE_INIT, "INSTANCE_INIT", first, LT(1))).add(cs_AST));
				currentAST.root = enumConstantField_AST;
	...
	protected final void enumConstantFieldInternal(AST mods, AST tp, AST t, Token first)
		...
		if (synPredMatched182) {
			...
			if (inputState.guessing == 0) {
				...
				enumConstantFieldInternal_AST = (AST) astFactory.make((new ASTArray(7))
						.add(create(METHOD_DEF, "METHOD", first, LT(1))).add(mods)
						.add((AST) astFactory.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t)))
						.add(tmp178_AST).add(param_AST).add(tc_AST).add(s2_AST));
				if (tp != null) {
				...
				currentAST.child = enumConstantFieldInternal_AST != null
						&& enumConstantFieldInternal_AST.getFirstChild() != null
	...
	/**
	 * A list of zero or more formal parameters. If a parameter is variable
	 * length (e.g. String... myArg) it should be to the right of any other
	 * parameters of the same kind. General form: (req, ..., opt, ..., [rest],
	 * key, ..., [restKeys], [block] This must be sorted out after parsing,
	 * since the various declaration forms are impossible to tell apart without
	 * backtracking.
	 */
	public final void parameterDeclarationList() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			parameterDeclarationList_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(PARAMETERS, "PARAMETERS", first, LT(1))).add(parameterDeclarationList_AST));
			currentAST.root = parameterDeclarationList_AST;
			currentAST.child = parameterDeclarationList_AST != null
					&& parameterDeclarationList_AST.getFirstChild() != null
	...
	/**
	 * I've split out constructors separately; we could maybe integrate back
	 * into variableDefinitions later on if we maybe simplified 'def' to be a
	 * type declaration?
	 */
	public final void constructorDefinition(AST mods) throws RecognitionException, TokenStreamException {
		...
		match(RPAREN);
		{
			...
			if (synPredMatched253) {
				throwsClause();
				tc_AST = (AST) returnAST;
			} else if ((LA(1) == LCURLY || LA(1) == NLS) && (_tokenSet_71.member(LA(2)))) {
			} else {
				throw new NoViableAltException(LT(1), getFilename());
			}
		...
		if (inputState.guessing == 0) {
			...
			constructorDefinition_AST = (AST) astFactory
					.make((new ASTArray(5)).add(create(CTOR_IDENT, "CTOR_IDENT", first, LT(1))).add(mods).add(param_AST)
							.add(tc_AST).add(cb_AST));
	...
	public final void multipleAssignmentDeclaration() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			multipleAssignmentDeclaration_AST = (AST) astFactory.make((new ASTArray(4))
					.add(create(VARIABLE_DEF, "VARIABLE_DEF", first, LT(1))).add(mods_AST)
					.add((AST) astFactory.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t_AST)))
					.add(multipleAssignmentDeclaration_AST));
			currentAST.root = multipleAssignmentDeclaration_AST;
			currentAST.child = multipleAssignmentDeclaration_AST != null
					&& multipleAssignmentDeclaration_AST.getFirstChild() != null
	...
	public final void constructorBody() throws RecognitionException, TokenStreamException {
		...
		nls();
		{
			...
			} else if ((_tokenSet_30.member(LA(1))) && (_tokenSet_74.member(LA(2)))) {
				blockBody(EOF);
		...
		if (inputState.guessing == 0) {
			...
			if (eci_AST != null)
				constructorBody_AST = (AST) astFactory
						.make((new ASTArray(3)).add(create(SLIST, "{", first, LT(1))).add(eci_AST).add(bb1_AST));
			else
				constructorBody_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(SLIST, "{", first, LT(1))).add(bb2_AST));
	...
	public final void listOfVariables(AST mods, AST t, Token first) throws RecognitionException, TokenStreamException {
		...
		variableDeclarator(getASTFactory().dupTree(mods), getASTFactory().dupTree(t), first);
		astFactory.addASTChild(currentAST, returnAST);
		{
			_loop225: do {
				if ((LA(1) == COMMA)) {
	...
	public final void variableDeclarator(AST mods, AST t, Token first)
		...
		if (inputState.guessing == 0) {
			...
			variableDeclarator_AST = (AST) astFactory.make((new ASTArray(5))
					.add(create(VARIABLE_DEF, "VARIABLE_DEF", first, LT(1))).add(mods)
					.add((AST) astFactory.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t)))
					.add(id_AST).add(v_AST));
	...
	/**
	 * Zero or more insignificant newlines, all gobbled up and thrown away, but
	 * a warning message is left for the user, if there was a newline.
	 */
	public final void nlsWarn() throws RecognitionException, TokenStreamException {
...

		{
			...
			if (synPredMatched604) {
				if (inputState.guessing == 0) {
					addWarning("A newline at this point does not follow the Groovy Coding Conventions.",
	...
	public final void expressionStatementNoCheck() throws RecognitionException, TokenStreamException {
		...
		{
			if (((_tokenSet_76.member(LA(1))) && (_tokenSet_53.member(LA(2)))) && (LA(1) != LITERAL_else
					&& isPathExpr /* && #head.getType()==METHOD_CALL */)) {
				...
				if (inputState.guessing == 0) {
					...
					currentAST.child = expressionStatementNoCheck_AST != null
							&& expressionStatementNoCheck_AST.getFirstChild() != null
	...
	public final void parameterDeclaration() throws RecognitionException, TokenStreamException {
		...
		pm_AST = (AST) returnAST;
		{
			if ((_tokenSet_24.member(LA(1))) && (_tokenSet_77.member(LA(2)))) {
				typeSpec(false);
				t_AST = (AST) returnAST;
			} else if ((LA(1) == IDENT || LA(1) == TRIPLE_DOT) && (_tokenSet_78.member(LA(2)))) {
			} else {
				throw new NoViableAltException(LT(1), getFilename());
			}
		...
		if (inputState.guessing == 0) {
			...
			if (spreadParam) {
				parameterDeclaration_AST = (AST) astFactory.make((new ASTArray(5))
						.add(create(VARIABLE_PARAMETER_DEF, "VARIABLE_PARAMETER_DEF", first, LT(1))).add(pm_AST)
						.add((AST) astFactory
								.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t_AST)))
						.add(id_AST).add(exp_AST));
			} else {
				parameterDeclaration_AST = (AST) astFactory
						.make((new ASTArray(5)).add(create(PARAMETER_DEF, "PARAMETER_DEF", first, LT(1))).add(pm_AST)
								.add((AST) astFactory
										.make((new ASTArray(2)).add(create(TYPE, "TYPE", first, LT(1))).add(t_AST)))
								.add(id_AST).add(exp_AST));
			}
	...
	public final void parameterModifiersOpt() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			parameterModifiersOpt_AST = (AST) astFactory.make(
					(new ASTArray(2)).add(create(MODIFIERS, "MODIFIERS", first, LT(1))).add(parameterModifiersOpt_AST));
	...
	public final void multicatch_types() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			multicatch_types_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(MULTICATCH_TYPES, "MULTICATCH_TYPES", first, LT(1))).add(multicatch_types_AST));
	...
	public final void multicatch() throws RecognitionException, TokenStreamException {
		...
		{
			if ((LA(1) == IDENT || LA(1) == NLS) && (_tokenSet_79.member(LA(2)))) {
				multicatch_types();
				m_AST = (AST) returnAST;
				astFactory.addASTChild(currentAST, returnAST);
		...
		if (inputState.guessing == 0) {
			...
			multicatch_AST = (AST) astFactory
					.make((new ASTArray(3)).add(create(MULTICATCH, "MULTICATCH", first, LT(1))).add(m_AST).add(id_AST));
	...
	/**
	 * Closure parameters are exactly like method parameters, except that they
	 * are not enclosed in parentheses, but rather are prepended to the front of
	 * a block, just after the brace. They are separated from the closure body
	 * by a CLOSABLE_BLOCK_OP token '->'.
	 */
	public final void closableBlockParamsOpt(boolean addImplicit) throws RecognitionException, TokenStreamException {
		...
		} else if (((_tokenSet_30.member(LA(1))) && (_tokenSet_53.member(LA(2)))) && (addImplicit)) {
			implicitParameters();
			astFactory.addASTChild(currentAST, returnAST);
			closableBlockParamsOpt_AST = (AST) currentAST.root;
		} else if ((_tokenSet_30.member(LA(1))) && (_tokenSet_53.member(LA(2)))) {
			closableBlockParamsOpt_AST = (AST) currentAST.root;
		} else {
			throw new NoViableAltException(LT(1), getFilename());
		}
	...
	/**
	 * A block known to be a closure, but which omits its arguments, is given
	 * this placeholder. A subsequent pass is responsible for deciding if there
	 * is an implicit 'it' parameter, or if the parameter list should be empty.
	 */
	public final void implicitParameters() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			implicitParameters_AST = (AST) astFactory
					.make((new ASTArray(1)).add(create(IMPLICIT_PARAMETERS, "IMPLICIT_PARAMETERS", first, LT(1))));
	...
	/**
	 * Lookahead to check whether a block begins with explicit closure
	 * arguments.
	 */
	...
	public final void closableBlockParam() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			closableBlockParam_AST = (AST) astFactory.make((new ASTArray(4))
					.add(create(PARAMETER_DEF, "PARAMETER_DEF", first, LT(1)))
					.add((AST) astFactory.make((new ASTArray(1)).add(create(MODIFIERS, "MODIFIERS", first, LT(1)))))
					.add((AST) astFactory.make((new ASTArray(1)).add(create(TYPE, "TYPE", first, LT(1))))).add(id_AST));
	...
	/**
	 * A block which is known to be a closure, even if it has no apparent
	 * arguments. A block inside an expression or after a method call is always
	 * assumed to be a closure. Only labeled, unparameterized blocks which occur
	 * directly as substatements are kept open.
	 */
	public final void closableBlock() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			closableBlock_AST = (AST) astFactory
					.make((new ASTArray(3)).add(create(CLOSABLE_BLOCK, "{", first, LT(1))).add(cbp_AST).add(bb_AST));
	...
	/**
	 * A sub-block of a block can be either open or closable. It is closable if
	 * and only if there are explicit closure arguments. Compare this to a block
	 * which is appended to a method call, which is given closure arguments,
	 * even if they are not explicit in the code.
	 */
	public final void openOrClosableBlock() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (cp_AST == null)
				openOrClosableBlock_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(SLIST, "{", first, LT(1))).add(bb_AST));
			else
				openOrClosableBlock_AST = (AST) astFactory
						.make((new ASTArray(3)).add(create(CLOSABLE_BLOCK, "{", first, LT(1))).add(cp_AST).add(bb_AST));
	...
	/**
	 * A labeled statement, consisting of a vanilla identifier followed by a
	 * colon.
	 */
	 ...
	 * <p>
	 * An expression statement can also be a <em>command</em>, which is a simple
	 * method call in which the outermost parentheses are omitted.
	 * <p>
	 * Certain "suspicious" looking forms are flagged for the user to
	 * disambiguate.
	 */
	public final void expressionStatement(int prevToken) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			expressionStatement_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(EXPR, "EXPR", first, LT(1))).add(esn_AST));
	...
	public final void assignmentLessExpression() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			assignmentLessExpression_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(EXPR, "EXPR", first, LT(1))).add(assignmentLessExpression_AST));
			currentAST.root = assignmentLessExpression_AST;
			currentAST.child = assignmentLessExpression_AST != null
					&& assignmentLessExpression_AST.getFirstChild() != null
	 ...
	 * statements as their bodies. Support this practice, even though it isn't
	 * very Groovy.
	 */
	public final void compatibleBodyStatement() throws RecognitionException, TokenStreamException {
		...
		if (synPredMatched337) {
			...
			if (synPredMatched340) {
				...
				if (inputState.guessing == 0) {
					compatibleBodyStatement_AST = (AST) currentAST.root;
					compatibleBodyStatement_AST = (AST) astFactory
							.make((new ASTArray(2)).add(create(SLIST, "CBSLIST", first, LT(1))).add(de_AST));
					currentAST.root = compatibleBodyStatement_AST;
					currentAST.child = compatibleBodyStatement_AST != null
							&& compatibleBodyStatement_AST.getFirstChild() != null
	...
	public final void forStatement() throws RecognitionException, TokenStreamException {
		...
		match(LPAREN);
		{
			...
			} else if ((_tokenSet_15.member(LA(1))) && (_tokenSet_84.member(LA(2)))) {
				forInClause();
		...
		if (inputState.guessing == 0) {
			...
			if (cl_AST != null) {
				if (s_AST != null)
					forStatement_AST = (AST) astFactory.make(
							(new ASTArray(3)).add(create(LITERAL_for, "for", first, LT(1))).add(cl_AST).add(s_AST));
				else
					forStatement_AST = (AST) astFactory.make((new ASTArray(3))
							.add(create(LITERAL_for, "for", first, LT(1))).add(cl_AST).add(forCbs_AST));
			} else {
				if (s_AST != null)
					forStatement_AST = (AST) astFactory.make(
							(new ASTArray(3)).add(create(LITERAL_for, "for", first, LT(1))).add(fic_AST).add(s_AST));
				else
					forStatement_AST = (AST) astFactory.make((new ASTArray(3))
							.add(create(LITERAL_for, "for", first, LT(1))).add(fic_AST).add(forCbs_AST));
			}
	...
	/**
	 * Things that can show up as expressions, but only in strict contexts like
	 * inside parentheses, argument lists, and list constructors.
	 */
	public final boolean strictContextExpression(boolean allowDeclaration)
		...
		if (inputState.guessing == 0) {
			...
			strictContextExpression_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(EXPR, "EXPR", first, LT(1))).add(strictContextExpression_AST));
			currentAST.root = strictContextExpression_AST;
			currentAST.child = strictContextExpression_AST != null
					&& strictContextExpression_AST.getFirstChild() != null ? strictContextExpression_AST.getFirstChild()
							: strictContextExpression_AST;
			currentAST.advanceChildToEnd();
		}
		strictContextExpression_AST = (AST) currentAST.root;
	...
	public final void casesGroup() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			casesGroup_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(CASE_GROUP, "CASE_GROUP", first, LT(1))).add(casesGroup_AST));
	...
	public final void tryBlock() throws RecognitionException, TokenStreamException {
		...
		{
			_loop381: do {
				if (((LA(1) == LITERAL_catch || LA(1) == NLS) && (LA(2) == LPAREN || LA(2) == LITERAL_catch))
						&& (!(LA(1) == NLS && LA(2) == LPAREN))) {
					nls();
					handler();
					h_AST = (AST) returnAST;
					if (inputState.guessing == 0) {
						newHandler_AST = (AST) astFactory
								.make((new ASTArray(3)).add(null).add(newHandler_AST).add(h_AST));
					}
				} else {
					break _loop381;
				}
		...
		if (inputState.guessing == 0) {
			...
			tryBlock_AST = (AST) astFactory.make((new ASTArray(4)).add(create(LITERAL_try, "try", first, LT(1)))
					.add(tryCs_AST).add(newHandler_AST).add(fc_AST));
	...
	/**
	 * In Groovy, return, break, continue, throw, and assert can be used in a
	 * parenthesized expression context. Example: println (x || (return));
	 * println assert x, "won't print a false value!" If an optional expression
	 * is missing, its value is void (this coerces to null when a value is
	 * required).
	 */
	public final void branchStatement() throws RecognitionException, TokenStreamException {
		...
		case LITERAL_return: {
			...
			if (inputState.guessing == 0) {
				...
				branchStatement_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(LITERAL_return, "return", first, LT(1))).add(returnE_AST));
		...
		case LITERAL_break: {
			...
			if (inputState.guessing == 0) {
				branchStatement_AST = (AST) currentAST.root;
				branchStatement_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(LITERAL_break, "break", first, LT(1))).add(breakI_AST));
				currentAST.root = branchStatement_AST;
				currentAST.child = branchStatement_AST != null && branchStatement_AST.getFirstChild() != null
						? branchStatement_AST.getFirstChild() : branchStatement_AST;
				currentAST.advanceChildToEnd();
		...
		case LITERAL_continue: {
			...
			if (inputState.guessing == 0) {
				branchStatement_AST = (AST) currentAST.root;
				branchStatement_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(LITERAL_continue, "continue", first, LT(1))).add(contI_AST));
				currentAST.root = branchStatement_AST;
				currentAST.child = branchStatement_AST != null && branchStatement_AST.getFirstChild() != null
						? branchStatement_AST.getFirstChild() : branchStatement_AST;
				currentAST.advanceChildToEnd();
		...
		case LITERAL_throw: {
			...
			if (inputState.guessing == 0) {
				branchStatement_AST = (AST) currentAST.root;
				branchStatement_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(LITERAL_throw, "throw", first, LT(1))).add(throwE_AST));
				currentAST.root = branchStatement_AST;
				currentAST.child = branchStatement_AST != null && branchStatement_AST.getFirstChild() != null
						? branchStatement_AST.getFirstChild() : branchStatement_AST;
				currentAST.advanceChildToEnd();
		...
		case LITERAL_assert: {
			...
			if (inputState.guessing == 0) {
				branchStatement_AST = (AST) currentAST.root;
				branchStatement_AST = (AST) astFactory.make((new ASTArray(3))
						.add(create(LITERAL_assert, "assert", first, LT(1))).add(assertAle_AST).add(assertE_AST));
				currentAST.root = branchStatement_AST;
				currentAST.child = branchStatement_AST != null && branchStatement_AST.getFirstChild() != null
						? branchStatement_AST.getFirstChild() : branchStatement_AST;
				currentAST.advanceChildToEnd();
	...
	public final void closureList() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			closureList_AST = (AST) astFactory.make(
					(new ASTArray(2)).add(create(CLOSURE_LIST, "CLOSURE_LIST", first, LT(1))).add(closureList_AST));
	...
	public final void forInClause() throws RecognitionException, TokenStreamException {
		...
		}
		{
			...
			case COLON: {
				if (inputState.guessing == 0) {
					addWarning("A colon at this point is legal Java but not recommended in Groovy.",
							"Use the 'in' keyword.");
					require(decl_AST != null, "Java-style for-each statement requires a type declaration.",
	 ...
	 * better not look like the latter half of an expression. If it does, issue
	 * a warning.
	 * <p>
	 * Also, if the expression starts with a closure, it needs to have an
	 * explicit parameter list, in order to avoid the appearance of a compound
	 * statement. This is a hard error.
	 * <p>
	 * These rules are different from Java's "dumb expression" restriction.
	 * Unlike Java, Groovy blocks can end with arbitrary (even dumb)
	 * expressions, as a consequence of optional 'return' and 'continue' tokens.
	 * <p>
	 * To make the programmer's intention clear, a leading closure must have an
	 * explicit parameter list, and must not follow a previous statement
		...
		if (synPredMatched356) {
			{
				if (((_tokenSet_19.member(LA(1))) && (_tokenSet_1.member(LA(2)))) && (prevToken == NLS)) {
					if (inputState.guessing == 0) {
						addWarning("Expression statement looks like it may continue a previous statement",
		...
		{
			{
				_loop400: do {
					if ((_tokenSet_93.member(LA(1))) && (_tokenSet_94.member(LA(2)))) {
						...
						if (inputState.guessing == 0) {
							prev = (AST) astFactory
									.make((new ASTArray(3)).add(create(DOT, ".", prev)).add(prev).add(pre_AST));
						}
						{
	...
	public final void caseSList() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			caseSList_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(SLIST, "SLIST", first, LT(1))).add(caseSList_AST));
	...
	public final void forInit() throws RecognitionException, TokenStreamException {
		...
		if (synPredMatched373) {
			...
			if (inputState.guessing == 0) {
				forInit_AST = (AST) currentAST.root;
				forInit_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(FOR_INIT, "FOR_INIT", first, LT(1))).add(forInit_AST));
	...
	public final void controlExpressionList() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			controlExpressionList_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(ELIST, "ELIST", first, LT(1))).add(controlExpressionList_AST));
	...
	public final void forCond() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			forCond_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(FOR_CONDITION, "FOR_CONDITION", first, LT(1))).add(forCond_AST));
			currentAST.root = forCond_AST;
			currentAST.child = forCond_AST != null && forCond_AST.getFirstChild() != null ? forCond_AST.getFirstChild()
					: forCond_AST;
			currentAST.advanceChildToEnd();
		}
		forCond_AST = (AST) currentAST.root;
	...
	public final void forIter() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			forIter_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(FOR_ITERATOR, "FOR_ITERATOR", first, LT(1))).add(forIter_AST));
			currentAST.root = forIter_AST;
			currentAST.child = forIter_AST != null && forIter_AST.getFirstChild() != null ? forIter_AST.getFirstChild()
					: forIter_AST;
			currentAST.advanceChildToEnd();
		}
		forIter_AST = (AST) currentAST.root;
	...
	public final void handler() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			handler_AST = (AST) astFactory.make(
					(new ASTArray(3)).add(create(LITERAL_catch, "catch", first, LT(1))).add(pd_AST).add(handlerCs_AST));
			currentAST.root = handler_AST;
			currentAST.child = handler_AST != null && handler_AST.getFirstChild() != null ? handler_AST.getFirstChild()
					: handler_AST;
			currentAST.advanceChildToEnd();
		}
		handler_AST = (AST) currentAST.root;
	...
	public final void finallyClause() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			finallyClause_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(LITERAL_finally, "finally", first, LT(1))).add(finallyCs_AST));
	 ...
	 * which may be followed by a list of arguments. Unlike parenthesized
	 * arguments, these must be plain expressions, without labels or spread
	 * operators.
	 */
	public final void commandArguments(AST head) throws RecognitionException, TokenStreamException {
		...
		{
			_loop387: do {
				if ((LA(1) == COMMA) && (_tokenSet_99.member(LA(2)))) {
					match(COMMA);
					nls();
					commandArgument();
					astFactory.addASTChild(currentAST, returnAST);
		...
		if (inputState.guessing == 0) {
			...
			AST elist = (AST) astFactory
					.make((new ASTArray(2)).add(create(ELIST, "ELIST", first, LT(1))).add(commandArguments_AST));
			AST headid = (AST) astFactory
					.make((new ASTArray(3)).add(create(METHOD_CALL, "<command>", first, LT(1))).add(head).add(elist));
	...
	public final void primaryExpression() throws RecognitionException, TokenStreamException {
		...
		case LPAREN: {
			...
			if (inputState.guessing == 0) {
				primaryExpression_AST = (AST) currentAST.root;
				primaryExpression_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(EXPR, "EXPR", first, LT(1))).add(pe_AST));
...

		{
			...
			_loop425: do {
				...
				if (synPredMatched422) {
					...
					if ((((LA(1) == LCURLY || LA(1) == NLS) && (_tokenSet_17.member(LA(2))))
	 ...
	 * except that the quotes allow more spellings. Equivalent dynamically
	 * computed labels are (('a'):b) and ("${'a'}":b) but not ((a):b) or "$a":b,
	 * since the latter cases evaluate (a) as a normal identifier. Bottom line:
	...
	public final void argumentLabel() throws RecognitionException, TokenStreamException {
		...
		if (synPredMatched574) {
			...
			if (((_tokenSet_102.member(LA(1))) && (LA(2) == COLON))) {
				int _m576 = mark();
			...
			if (synPredMatched576) {
				...
				argumentLabel_AST = (AST) currentAST.root;
			} else if ((_tokenSet_93.member(LA(1))) && (_tokenSet_101.member(LA(2)))) {
				primaryExpression();
				astFactory.addASTChild(currentAST, returnAST);
				argumentLabel_AST = (AST) currentAST.root;
			} else {
	...
	public final void pathElement(AST prefix) throws RecognitionException, TokenStreamException {
		...
		case NLS: {
			...
			if (inputState.guessing == 0) {
				pathElement_AST = (AST) currentAST.root;
				pathElement_AST = (AST) astFactory
						.make((new ASTArray(4)).add(create(operator.getType(), operator.getText(), prefix, LT(1)))
								.add(prefix).add(ta_AST).add(np_AST));
				currentAST.root = pathElement_AST;
				currentAST.child = pathElement_AST != null && pathElement_AST.getFirstChild() != null
						? pathElement_AST.getFirstChild() : pathElement_AST;
				currentAST.advanceChildToEnd();
	...
	/**
	 * An appended block follows any expression. If the expression is not a
	 * method call, it is given an empty argument list.
	 */
	public final void appendedBlock(AST callee) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (callee != null && callee.getType() == METHOD_CALL) {
				appendedBlock_AST = (AST) astFactory.make((new ASTArray(3)).add(create(METHOD_CALL, "(", callee, LT(1)))
						.add(callee.getFirstChild()).add(cb_AST));
			} else {
				appendedBlock_AST = (AST) astFactory
						.make((new ASTArray(3)).add(create(METHOD_CALL, "{", callee, LT(1))).add(callee).add(cb_AST));
			}
	...
	/**
	 * A "path expression" is a name or other primary, possibly qualified by
	 * various forms of dot, and/or followed by various kinds of brackets. It
	 * can be used for value or assigned to, or else further qualified, indexed,
	 * or called. It is called a "path" because it looks like a linear path
	 * through a data structure. Examples: x.y, x?.y, x*.y, x.@y; x[], x[y],
	 * x[y,z]; x(), x(y), x(y,z); x{s}; a.b[n].c(x).d{s} (Compare to a C lvalue,
	 * or LeftHandSide in the JLS section 15.26.) General expressions are built
	 * up from path expressions, using operators like '+' and '='.
	 */
	...
	/**
	 * This is the grammar for what can follow a dot: x.a, x.@a, x.&a, x.'a',
	 * etc. Note: <code>typeArguments</code> is handled by the caller of
	 * <code>namePart</code>.
	 */
	...
	/**
	 * An expression may be followed by one or both of (...) and {...}. Note: If
	 * either is (...) or {...} present, it is a method call. The {...} is
	 * appended to the argument list, and matches a formal of type Closure. If
	 * there is no method member, a property (or field) is used instead, and
	 * must itself be callable.
	 * <p>
	 * If the methodCallArgs are absent, it is a property reference. If there is
	 * no property, it is treated as a field reference, but never a method
	 * reference.
	 * <p>
	 * Arguments in the (...) can be labeled, and the appended block can be
	 * labeled also. If there is a mix of unlabeled and labeled arguments, all
	 * the labeled arguments must follow the unlabeled arguments, except that
	 * the closure (labeled or not) is always a separate final argument. Labeled
	 * arguments are collected up and passed as a single argument to a formal of
	 * type Map.
	 * <p>
	 * Therefore, f(x,y, a:p, b:q) {s} is equivalent in all ways to f(x,y,
	 * [a:p,b:q], {s}). Spread arguments of sequence type count as unlabeled
	 * arguments, while spread arguments of map type count as labeled arguments.
	 * (This distinction must sometimes be checked dynamically.)
	 *
	 * A plain unlabeled argument is allowed to match a trailing Map or Closure
	 * argument: f(x, a:p) {s} === f(*[ x, [a:p], {s} ])
	 */
	public final void methodCallArgs(AST callee) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (callee != null && callee.getFirstChild() != null) {
				...
				methodCallArgs_AST = (AST) astFactory.make((new ASTArray(3))
						.add(create(METHOD_CALL, "(", callee.getFirstChild(), LT(1))).add(callee).add(al_AST));
			} else {
				// method call like method() or new Expr(), in the latter case
				// "callee" is null
				methodCallArgs_AST = (AST) astFactory
						.make((new ASTArray(3)).add(create(METHOD_CALL, "(", callee, LT(1))).add(callee).add(al_AST));
			}
	...
	/**
	 * An expression may be followed by [...]. Unlike Java, these brackets may
	 * contain a general argument list, which is passed to the array type
	 * operator, which can make of it what it wants. The brackets may also be
	 * empty, as in T[]. This is how Groovy names array types.
	 * <p>
	...
	public final void indexPropertyArgs(AST indexee) throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (indexee != null && indexee.getFirstChild() != null) {
				...
				indexPropertyArgs_AST = (AST) astFactory
						.make((new ASTArray(4)).add(create(INDEX_OP, "INDEX_OP", indexee.getFirstChild(), LT(1)))
								.add(lb_AST).add(indexee).add(al_AST));
			} else {
				// expression like obj[]
				indexPropertyArgs_AST = (AST) astFactory.make((new ASTArray(4))
						.add(create(INDEX_OP, "INDEX_OP", indexee, LT(1))).add(lb_AST).add(indexee).add(al_AST));
			}
	...
	/**
	 * If a dot is followed by a parenthesized or quoted expression, the member
	 * is computed dynamically, and the member selection is done only at
	 * runtime. This forces a statically unchecked member access.
	 */
	public final void dynamicMemberName() throws RecognitionException, TokenStreamException {
...

		{
			...
			case LPAREN: {
				...
				if (inputState.guessing == 0) {
					...
					dynamicMemberName_AST = (AST) astFactory
							.make((new ASTArray(2)).add(create(EXPR, "EXPR", first, LT(1))).add(pe_AST));
		...
		if (inputState.guessing == 0) {
			dynamicMemberName_AST = (AST) currentAST.root;
			dynamicMemberName_AST = (AST) astFactory.make((new ASTArray(2))
					.add(create(DYNAMIC_MEMBER, "DYNAMIC_MEMBER", first, LT(1))).add(dynamicMemberName_AST));
			currentAST.root = dynamicMemberName_AST;
			currentAST.child = dynamicMemberName_AST != null && dynamicMemberName_AST.getFirstChild() != null
					? dynamicMemberName_AST.getFirstChild() : dynamicMemberName_AST;
			currentAST.advanceChildToEnd();
	...
	public final void parenthesizedExpression() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			if (hasClosureList) {
				parenthesizedExpression_AST = (AST) astFactory.make((new ASTArray(2))
						.add(create(CLOSURE_LIST, "CLOSURE_LIST", first, LT(1))).add(parenthesizedExpression_AST));
			}

			currentAST.root = parenthesizedExpression_AST;
			currentAST.child = parenthesizedExpression_AST != null
					&& parenthesizedExpression_AST.getFirstChild() != null ? parenthesizedExpression_AST.getFirstChild()
							: parenthesizedExpression_AST;
			currentAST.advanceChildToEnd();
		}
		parenthesizedExpression_AST = (AST) currentAST.root;
	...
	public final void stringConstructorExpression() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			stringConstructorExpression_AST = (AST) astFactory
					.make((new ASTArray(2)).add(create(STRING_CONSTRUCTOR, "STRING_CONSTRUCTOR", first, LT(1)))
							.add(stringConstructorExpression_AST));

			currentAST.root = stringConstructorExpression_AST;
			currentAST.child = stringConstructorExpression_AST != null
					&& stringConstructorExpression_AST.getFirstChild() != null
	...
	public final void relationalExpression(int lc_stmt) throws RecognitionException, TokenStreamException {
		...
		astFactory.addASTChild(currentAST, returnAST);
		{
			if ((_tokenSet_103.member(LA(1))) && (_tokenSet_87.member(LA(2)))) {
			...
			} else if ((LA(1) == LITERAL_instanceof) && (_tokenSet_104.member(LA(2)))) {
				AST tmp302_AST = null;
				...
				typeSpec(true);
				astFactory.addASTChild(currentAST, returnAST);
			} else if ((LA(1) == LITERAL_as) && (_tokenSet_104.member(LA(2)))) {
				AST tmp303_AST = null;
		...
		case NUM_BIG_DECIMAL: {
			{
				...
				if (synPredMatched526) {
					...
					if (synPredMatched528) {
						...
						unaryExpressionNotPlusMinus(0);
						astFactory.addASTChild(currentAST, returnAST);
					} else if ((_tokenSet_93.member(LA(1))) && (_tokenSet_37.member(LA(2)))) {
	...
	public final void postfixExpression(int lc_stmt) throws RecognitionException, TokenStreamException {
		...
		astFactory.addASTChild(currentAST, returnAST);
		{
			if ((LA(1) == INC) && (_tokenSet_107.member(LA(2)))) {
				in = LT(1);
			...
			} else if ((LA(1) == DEC) && (_tokenSet_107.member(LA(2)))) {
				de = LT(1);
	...
	public final void newExpression() throws RecognitionException, TokenStreamException {
		...
		t_AST = (AST) returnAST;
		{
			...
			case NLS: {
				...
				mca_AST = (AST) returnAST;
				{
					if ((LA(1) == LCURLY) && (_tokenSet_55.member(LA(2)))) {
						classBlock();
						cb_AST = (AST) returnAST;
						astFactory.addASTChild(currentAST, returnAST);
				...
				if (inputState.guessing == 0) {
					...
					newExpression_AST = (AST) astFactory
							.make((new ASTArray(5)).add(create(LITERAL_new, "new", first, LT(1))).add(ta_AST).add(t_AST)
									.add(mca_AST).add(cb_AST));
			...
			case LBRACK: {
				...
				if (inputState.guessing == 0) {
					newExpression_AST = (AST) currentAST.root;
					newExpression_AST = (AST) astFactory.make((new ASTArray(4))
							.add(create(LITERAL_new, "new", first, LT(1))).add(ta_AST).add(t_AST).add(ad_AST));
					currentAST.root = newExpression_AST;
					currentAST.child = newExpression_AST != null && newExpression_AST.getFirstChild() != null
							? newExpression_AST.getFirstChild() : newExpression_AST;
					currentAST.advanceChildToEnd();
	...
	/**
	 * A list constructor is a argument list enclosed in square brackets,
	 * without labels. Any argument can be decorated with a spread operator
	 * (*x), but not a label (a:x). Examples: [], [1], [1,2], [1,*l1,2],
	 * [*l1,*l2]. (The l1, l2 must be a sequence or null.)
	 * <p>
	 * A map constructor is an argument list enclosed in square brackets, with
	 * labels everywhere, except on spread arguments, which stand for whole maps
	 * spliced in. A colon alone between the brackets also forces the expression
	 * to be an empty map constructor. Examples: [:], [a:1], [a:1,b:2],
	 * [a:1,*:m1,b:2], [*:m1,*:m2] (The m1, m2 must be a map or null.) Values
	 * associated with identical keys overwrite from left to right: [a:1,a:2]
	 * === [a:2]
	 * <p>
	 * Some malformed constructor expressions are not detected in the parser,
	 * but in a post-pass. Bad examples: [1,b:2], [a:1,2], [:1]. (Note that
	 * method call arguments, by contrast, can be a mix of keyworded and
	...
	public final void listOrMapConstructorExpression() throws RecognitionException, TokenStreamException {
		...
		if ((LA(1) == LBRACK) && (_tokenSet_109.member(LA(2)))) {
			lcon = LT(1);
			...
			if (inputState.guessing == 0) {
				...
				listOrMapConstructorExpression_AST = (AST) astFactory
						.make((new ASTArray(2)).add(create(type, "[", lcon_AST, LT(1))).add(args_AST));

				currentAST.root = listOrMapConstructorExpression_AST;
				currentAST.child = listOrMapConstructorExpression_AST != null
						&& listOrMapConstructorExpression_AST.getFirstChild() != null
								? listOrMapConstructorExpression_AST.getFirstChild()
								: listOrMapConstructorExpression_AST;
				currentAST.advanceChildToEnd();
			}
			listOrMapConstructorExpression_AST = (AST) currentAST.root;
	...
	/**
	 * A single argument in (...) or [...]. Corresponds to to a method or
	 * closure parameter. May be labeled. May be modified by the spread operator
	 * '*' ('*:' for keywords).
	 */
	public final byte argument() throws RecognitionException, TokenStreamException {
		...
		if (inputState.guessing == 0) {
			...
			require(LA(1) != COLON, "illegal colon after argument expression",
	...
	/**
	 * For lookahead only. Fast approximate parse of an argumentLabel followed
	 * by a colon.
	 */
	...
	public static final String[] _tokenNames = { "<0>", "EOF", "<2>", "NULL_TREE_LOOKAHEAD", "BLOCK", "MODIFIERS",
			"OBJBLOCK", "SLIST", "METHOD", "VARIABLE_DEF", "INSTANCE_INIT", "STATIC_INIT", "TYPE", "CLASS_DEF",
			"INTERFACE_DEF", "TRAIT_DEF", "PACKAGE_DEF", "ARRAY_DECLARATOR", "EXTENDS_CLAUSE", "IMPLEMENTS_CLAUSE",
			"PARAMETERS", "PARAMETER_DEF", "LABELED_STAT", "TYPECAST", "INDEX_OP", "POST_INC", "POST_DEC",
			"METHOD_CALL", "EXPR", "IMPORT", "UNARY_MINUS", "UNARY_PLUS", "CASE_GROUP", "ELIST", "FOR_INIT",
			"FOR_CONDITION", "FOR_ITERATOR", "EMPTY_STAT", "\"final\"", "\"abstract\"", "\"goto\"", "\"const\"",
			"\"do\"", "\"strictfp\"", "SUPER_CTOR_CALL", "CTOR_CALL", "CTOR_IDENT", "VARIABLE_PARAMETER_DEF",
			"STRING_CONSTRUCTOR", "STRING_CTOR_MIDDLE", "CLOSABLE_BLOCK", "IMPLICIT_PARAMETERS", "SELECT_SLOT",
			"DYNAMIC_MEMBER", "LABELED_ARG", "SPREAD_ARG", "SPREAD_MAP_ARG", "LIST_CONSTRUCTOR", "MAP_CONSTRUCTOR",
			"FOR_IN_ITERABLE", "STATIC_IMPORT", "ENUM_DEF", "ENUM_CONSTANT_DEF", "FOR_EACH_CLAUSE", "ANNOTATION_DEF",
			"ANNOTATIONS", "ANNOTATION", "ANNOTATION_MEMBER_VALUE_PAIR", "ANNOTATION_FIELD_DEF",
			"ANNOTATION_ARRAY_INIT", "TYPE_ARGUMENTS", "TYPE_ARGUMENT", "TYPE_PARAMETERS", "TYPE_PARAMETER",
			"WILDCARD_TYPE", "TYPE_UPPER_BOUNDS", "TYPE_LOWER_BOUNDS", "CLOSURE_LIST", "MULTICATCH", "MULTICATCH_TYPES",
			"a script header", "\"package\"", "\"import\"", "\"static\"", "\"def\"", "'['", "']'", "an identifier",
			"a string literal", "'<'", "'.'", "'('", "\"class\"", "\"interface\"", "\"enum\"", "\"trait\"", "'@'",
			"'?'", "\"extends\"", "\"super\"", "'>'", "','", "'>>'", "'>>>'", "\"void\"", "\"boolean\"", "\"byte\"",
			"\"char\"", "\"short\"", "\"int\"", "\"float\"", "\"long\"", "\"double\"", "'*'", "\"as\"", "\"private\"",
			"\"public\"", "\"protected\"", "\"transient\"", "\"native\"", "\"threadsafe\"", "\"synchronized\"",
			"\"volatile\"", "')'", "'='", "'&'", "'{'", "'}'", "';'", "\"default\"", "\"throws\"", "\"implements\"",
			"\"this\"", "'...'", "'|'", "'->'", "':'", "\"if\"", "\"else\"", "\"while\"", "\"switch\"", "\"for\"",
			"\"in\"", "\"return\"", "\"break\"", "\"continue\"", "\"throw\"", "\"assert\"", "'+'", "'-'", "\"case\"",
			"\"try\"", "\"finally\"", "\"catch\"", "'*.'", "'?.'", "'.&'", "\"false\"", "\"instanceof\"", "\"new\"",
			"\"null\"", "\"true\"", "'+='", "'-='", "'*='", "'/='", "'%='", "'>>='", "'>>>='", "'<<='", "'&='", "'^='",
			"'|='", "'**='", "'?:'", "'||'", "'&&'", "'^'", "'=~'", "'==~'", "'!='", "'=='", "'==='", "'!=='", "'<=>'",
			"'<='", "'>='", "'<<'", "'..'", "'..<'", "'++'", "'/'", "'%'", "'--'", "'**'", "'~'", "'!'",
			"STRING_CTOR_START", "a string literal end", "a numeric literal", "NUM_FLOAT", "NUM_LONG", "NUM_DOUBLE",
			"NUM_BIG_INT", "NUM_BIG_DECIMAL", "some newlines, whitespace or comments", "'$'", "whitespace", "a newline",
			"a single line comment", "a multi-line comment", "a string character",
			"a multiline regular expression literal", "a multiline dollar escaping regular expression literal",
			"a multiline regular expression literal end", "a multiline dollar escaping regular expression literal end",
			"ESCAPED_SLASH", "ESCAPED_DOLLAR", "a multiline regular expression character",
			"a multiline dollar escaping regular expression character", "an escape sequence",
			"a newline inside a string", "a hexadecimal digit", "a character", "a letter", "a digit",
			"a sequence of digits and underscores, bordered by digits",
			"a sequence of digits and underscores with maybe underscore starting", "an exponent",
			"a float or double suffix", "a big decimal suffix" };

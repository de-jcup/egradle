        ...
        return new TokenStream() {
            public Token nextToken() throws TokenStreamException {
                if (stringCtorState >= SCS_LIT) {
                    ...
                    resetText();
                    /*
                     * try { switch (quoteType) { case SCS_SQ_TYPE: //todo: suitable replacement???
                     * mSTRING_CTOR_END(true, false, false); break; case SCS_TQ_TYPE: //
                     * mSTRING_CTOR_END(true, false, true); break; case SCS_RE_TYPE: //
                     * mREGEXP_CTOR_END(true, false); break; default: throw new
                     * AssertionError(false); } lastSigTokenType = _returnToken.getType(); return
                     * _returnToken; }
                     *//*
                        * catch (RecognitionException e) { throw new
                        * TokenStreamRecognitionException(e); }
                        */ /*
                            * catch (CharStreamException cse) { if ( cse instanceof CharStreamIOException )
                            * { throw new TokenStreamIOException(((CharStreamIOException) cse).io); } else

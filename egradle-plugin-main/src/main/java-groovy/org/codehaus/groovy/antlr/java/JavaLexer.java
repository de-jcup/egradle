...
public class JavaLexer extends antlr.CharScanner implements JavaTokenTypes, TokenStream {
	 ...
	 * JavaRecognizer(new JavaLexer(in).plumb()).
	 */
		...
		return new TokenStream() {
			public Token nextToken() throws TokenStreamException {
				if (stringCtorState >= SCS_LIT) {
					...
					resetText();
					/*
					 * try { switch (quoteType) { case SCS_SQ_TYPE: //todo:
					 * suitable replacement??? mSTRING_CTOR_END(true, false,
					 * false); break; case SCS_TQ_TYPE: //
					 * mSTRING_CTOR_END(true, false, true); break; case
					 * SCS_RE_TYPE: // mREGEXP_CTOR_END(true, false); break;
					 * default: throw new AssertionError(false); }
					 * lastSigTokenType = _returnToken.getType(); return
					 * _returnToken; }
					 *//*
						 * catch (RecognitionException e) { throw new
						 * TokenStreamRecognitionException(e); }
						 */ /*
							 * catch (CharStreamException cse) { if ( cse
							 * instanceof CharStreamIOException ) { throw new
							 * TokenStreamIOException(((CharStreamIOException)
							 * cse).io); } else { throw new
	...
	public final void mNUM_INT(boolean _createToken)
		...
		case '9': {
			...
			}
			{
				...
				} else if (((LA(1) == '.' || LA(1) == 'D' || LA(1) == 'E' || LA(1) == 'F' || LA(1) == 'd'
						|| LA(1) == 'e' || LA(1) == 'f')) && (isDecimal)) {
					{
						switch (LA(1)) {
						case '.': {
							match('.');

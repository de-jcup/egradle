...
public class UnicodeEscapingReader extends Reader {
	...
	/**
	 * Sets the lexer that is using this reader. Must be called before the lexer
	 * is used.
	 */
	...
	/**
	 * Gets the next character from the underlying reader, translating escapes
	 * as required.
	 * 
	...
	private void checkHexDigit(int c) throws IOException {
		...
		hasNextChar = true;
		nextChar = c;
		throw new IOException("Did not find four digit hex character code." + " line: " + lexer.getLine() + " col:"
				+ lexer.getColumn());
	}

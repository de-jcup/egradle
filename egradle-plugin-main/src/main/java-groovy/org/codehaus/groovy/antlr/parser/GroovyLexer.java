...
public class GroovyLexer extends antlr.CharScanner implements GroovyTokenTypes, TokenStream {
...

	{
		...
		setTabSize(1); // get rid of special tab interpretation, for IDEs and
						// general clarity
	}

	/** Bumped when inside '[x]' or '(x)', reset inside '{x}'. See ONE_NL. */
	protected int parenLevel = 0;
	protected int suppressNewline = 0; // be really mean to newlines inside
										// strings
	...
	protected int stringCtorState = 0; // hack string and regexp constructor
										// boundaries
	 ...
	 * Return true for keywords, identifiers, and literals. Return true for
	 * tokens which can end expressions (right brackets, ++, --). Return false
	 * for EOF and all other operator and punctuation tokens. Used to suppress
	 * the recognition of /foo/ as opposed to the simple division operator '/'.
	 */
	...
	protected static boolean isExpressionEndingToken(int ttype) {
		...
		case IDENT: // x / y
			// and a bunch of keywords (all of them; no sense picking and
			// choosing):
		case LITERAL_as:
		case LITERAL_assert:
	...
	protected void newlineCheck(boolean check) throws RecognitionException {
		if (check && suppressNewline > 0) {
			require(suppressNewline == 0, "end of line reached within a simple string 'x' or \"x\" or /x/",
	...
	/**
	 * This is a bit of plumbing which resumes collection of string constructor
	 * bodies, after an embedded expression has been parsed. Usage: new
	 * GroovyRecognizer(new GroovyLexer(in).plumb()).
	 */
			...
			public Token nextToken() throws TokenStreamException {
				if (stringCtorState >= SCS_LIT) {
					...
					try {
						switch (quoteType) {
						case SCS_SQ_TYPE:
							mSTRING_CTOR_END(true, /* fromStart: */false, false);
							break;
						case SCS_TQ_TYPE:
							mSTRING_CTOR_END(true, /* fromStart: */false, true);
							break;
						case SCS_RE_TYPE:
							mREGEXP_CTOR_END(true, /* fromStart: */false);
							break;
						case SCS_DRE_TYPE:
							mDOLLAR_REGEXP_CTOR_END(true, /* fromStart: */false);
							break;
						default:
	...
	public void traceOut(String rname) throws CharStreamException {
		...
		if (_returnToken != null)
			rname += tokenStringOf(_returnToken);
	...
	private void require(boolean z, String problem, String solution) throws SemanticException {
		// TODO: Direct to a common error outputHandler, rather than through the
		// parser.
		if (!z && parser != null)
			parser.requireFailed(problem, solution);
		if (!z) {
	...
	public final void mNLS(boolean _createToken)
		...
		mONE_NL(false, true);
		{
			if (((LA(1) == '\t' || LA(1) == '\n' || LA(1) == '\u000c' || LA(1) == '\r' || LA(1) == ' ' || LA(1) == '/'
					|| LA(1) == '\\')) && (!whitespaceIncluded)) {
				{
					int _cnt675 = 0;
					_loop675: do {
						switch (LA(1)) {
	...
	public final void mML_COMMENT(boolean _createToken)
		...
		{
			_loop689: do {
				boolean synPredMatched687 = false;
				if (((LA(1) == '*') && ((LA(2) >= '\u0000' && LA(2) <= '\ufffe'))
						&& ((LA(3) >= '\u0000' && LA(3) <= '\ufffe')) && (true))) {
					int _m687 = mark();
					synPredMatched687 = true;
					inputState.guessing++;
					try {
						{
							match('*');
						...
						synPredMatched687 = false;
					}
					rewind(_m687);
					inputState.guessing--;
				}
				if (synPredMatched687) {
					match('*');
				} else if ((LA(1) == '\n' || LA(1) == '\r')) {
					mONE_NL(false, true);
	...
	protected final void mESC(boolean _createToken)
		...
		if ((LA(1) == '\\') && (LA(2) == '"' || LA(2) == '$' || LA(2) == '\'' || LA(2) == '0' || LA(2) == '1'
				|| LA(2) == '2' || LA(2) == '3' || LA(2) == '4' || LA(2) == '5' || LA(2) == '6' || LA(2) == '7'
				|| LA(2) == '\\' || LA(2) == 'b' || LA(2) == 'f' || LA(2) == 'n' || LA(2) == 'r' || LA(2) == 't'
				|| LA(2) == 'u')) {
			_saveIndex = text.length();
			match('\\');
			text.setLength(_saveIndex);
			{
				...
				case 'u': {
					...
					mHEX_DIGIT(false);
					if (inputState.guessing == 0) {
						char ch = (char) Integer.parseInt(new String(text.getBuffer(), _begin, text.length() - _begin),
								16);
						text.setLength(_begin);
						text.append(ch);
					}
		...
		}
		{
			...
			case '"': {
				...
				if (inputState.guessing == 0) {
					...
					if (fromStart)
						tt = STRING_LITERAL; // plain string literal!
					if (!tripleQuote) {
						--suppressNewline;
					}
			...
			case '$': {
				...
				if (inputState.guessing == 0) {
					...
					require(dollarOK, "illegal string body character after dollar sign",
							"either escape a literal dollar sign \"\\$5\" or bracket the value expression \"${5}\"");
					// Yes, it's a string constructor, and we've got a value
					// part.
				...
				}
				{
					if (((LA(1) == '$') && ((LA(2) >= '\u0000' && LA(2) <= '\ufffe'))) && (!atValidDollarEscape())) {
						...
						if (inputState.guessing == 0) {
							...
							// Yes, it's a regexp constructor, and we've got a
							// value part.
		...
		}
		{
			...
			case '/': {
				...
				if (inputState.guessing == 0) {
					...
					if (fromStart)
						tt = STRING_LITERAL; // plain regexp literal!
					{
						--suppressNewline;
					}
			...
			case '$': {
				...
				if (inputState.guessing == 0) {
					...
					// Yes, it's a regexp constructor, and we've got a value
					// part.
					tt = (fromStart ? STRING_CTOR_START : STRING_CTOR_MIDDLE);
					stringCtorState = SCS_VAL + SCS_RE_TYPE;
			...
			_loop733: do {
				...
				if (synPredMatched730) {
					...
					if (synPredMatched732) {
						mESCAPED_DOLLAR(false);
					} else if (((_tokenSet_9.member(LA(1))) && ((LA(2) >= '\u0000' && LA(2) <= '\ufffe')) && (true)
							&& (true)) && (!(LA(1) == '/' && LA(2) == '$'))) {
						mDOLLAR_REGEXP_SYMBOL(false);
					} else if (((LA(1) == '$') && ((LA(2) >= '\u0000' && LA(2) <= '\ufffe')) && (true) && (true))
							&& (!atValidDollarEscape() && !atDollarSlashEscape() && !atDollarDollarEscape())) {
		...
		if (inputState.guessing == 0) {
			...
			if (stringCtorState != 0) {
				if (LA(1) == '.' && LA(2) != '$' && Character.isJavaIdentifierStart(LA(2))) {
					// pick up another name component before going literal
					// again:
			...
			// Java doesn't have the keywords 'as', 'in' or 'def so we make some
			// allowances
			// for them in package names for better integration with existing
			// Java packages
			...
			if ((ttype == LITERAL_package) && (LA(1) == '.' || lastSigTokenType == DOT
					|| lastSigTokenType == LITERAL_import || (LA(1) == ')' && lastSigTokenType == LPAREN))) {
				ttype = IDENT;
			}
			...
			if (assertEnabled && "assert".equals(new String(text.getBuffer(), _begin, text.length() - _begin))) {
				_ttype = LITERAL_assert; // set token type for the rule in the
											// parser
			}
			// check if "enum" keyword is enabled
			if (enumEnabled && "enum".equals(new String(text.getBuffer(), _begin, text.length() - _begin))) {
				_ttype = LITERAL_enum; // set token type for the rule in the
										// parser
			}
	...
	protected final void mDIGITS_WITH_UNDERSCORE(boolean _createToken)
		...
		mDIGIT(false);
		{
			if ((LA(1) == '0' || LA(1) == '1' || LA(1) == '2' || LA(1) == '3' || LA(1) == '4' || LA(1) == '5'
					|| LA(1) == '6' || LA(1) == '7' || LA(1) == '8' || LA(1) == '9' || LA(1) == '_')) {
	...
	protected final void mDIGITS_WITH_UNDERSCORE_OPT(boolean _createToken)
...

		{
			_loop767: do {
				if (((LA(1) >= '0' && LA(1) <= '9'))
						&& (LA(2) == '0' || LA(2) == '1' || LA(2) == '2' || LA(2) == '3' || LA(2) == '4' || LA(2) == '5'
								|| LA(2) == '6' || LA(2) == '7' || LA(2) == '8' || LA(2) == '9' || LA(2) == '_')) {
					mDIGIT(false);
				} else if ((LA(1) == '_')) {
					match('_');
				} else {
	...
	public final void mNUM_INT(boolean _createToken)
				...
				}
				{
					...
					case 'x': {
						...
						mHEX_DIGIT(false);
						{
							if ((_tokenSet_11.member(LA(1))) && (true) && (true) && (true)) {
								{
									_loop774: do {
										if ((_tokenSet_12.member(LA(1))) && (_tokenSet_11.member(LA(2))) && (true)
												&& (true)) {
											mHEX_DIGIT(false);
										} else if ((LA(1) == '_')) {
											match('_');
					...
					default:
						...
						if (synPredMatched783) {
							...
							{
								if ((LA(1) == '0' || LA(1) == '1' || LA(1) == '2' || LA(1) == '3' || LA(1) == '4'
										|| LA(1) == '5' || LA(1) == '6' || LA(1) == '7' || LA(1) == '_')) {
									{
										_loop787: do {
											if (((LA(1) >= '0' && LA(1) <= '7')) && (LA(2) == '0' || LA(2) == '1'
													|| LA(2) == '2' || LA(2) == '3' || LA(2) == '4' || LA(2) == '5'
													|| LA(2) == '6' || LA(2) == '7' || LA(2) == '_')) {
												matchRange('0', '7');
											} else if ((LA(1) == '_')) {
												match('_');
											} else {
		...
		}
		{
			...
			default:
				...
				if ((((LA(1) == '.' || LA(1) == 'D' || LA(1) == 'E' || LA(1) == 'F' || LA(1) == 'd' || LA(1) == 'e'
						|| LA(1) == 'f')) && (isDecimal))) {
					int _m796 = mark();

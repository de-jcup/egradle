 ...
 * 
 * @param <T> generic typ of tree element items
...
public abstract class AbstractFilterableTreeQuickDialog<T> extends AbstractQuickDialog implements IDoubleClickListener {
     ...
     * 
     * @param adaptable adaptable which can be used by child class implementations
     ...
     * @param minWidth
     * @param minHeight
     * @param infoText  additional information to show at the bottom of dialogs
     */
    public AbstractFilterableTreeQuickDialog(IAdaptable adaptable, Shell parent, String title, int minWidth, int minHeight, String infoText) {
        super(parent, PopupDialog.INFOPOPUPRESIZE_SHELLSTYLE, GRAB_FOCUS, PERSIST_SIZE, PERSIST_BOUNDS, DO_SHOW_DIALOG, SHOW_PERSIST_ACTIONS, title, infoText);
     ...
     * 
     * 
     * @param filterText the filter as text or <code>null</code> if not filtered
    ...
    protected Control createTitleControl(Composite parent) {
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).span(DO_SHOW_DIALOG ? 1 : 2, 1).applyTo(text);

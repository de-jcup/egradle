...
public abstract class AbstractGroovySourceViewerConfiguration extends TextSourceViewerConfiguration implements ExtendedSourceViewerConfiguration {
    ...
    public AbstractGroovySourceViewerConfiguration(IAdaptable adaptable, PreferenceIdentifiable colorNormalText) {
        ...
        this.defaultTextAttribute = new TextAttribute(colorManager.getColor(getPreferences().getColor(colorNormalText)));

    }

    /**
     * Returns preferences laber with its preference store. The returned store is
     * containing only preferences for this editor - but has no relation to standard
     * text editor setup.
     * 
     * @return
     */

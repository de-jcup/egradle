...
public abstract class AbstractMarkerHelper {
    ...
    private IMarker findMarker(IResource resource, String message, int lineNumber, String type) throws CoreException {
        ...
        for (int i = 0; i < marker.length; i++) {
            ...
            boolean sameMessageAndLineNr = StringUtils.equals(markerLineNumber, String.valueOf(lineNumber)) && StringUtils.equals(markerMessage, message);
     ...
     * @param message
     * @param lineNumber
     * @param charStart  if not needed use -1
     * @param charEnd    if not needed use -1
     * @throws CoreException
     */
    public void createErrorMarker(IResource resource, String message, int lineNumber, int charStart, int charEnd) throws CoreException {
     ...
     * @param resource
     * @param message
     * @param lineNumber numbers maller than 1 will be transformed to 1
     * @param markerType
     * @param severity   e.g. {@link IMarker#SEVERITY_ERROR}
     * @param charStart  if not needed use -1
     * @param charEnd    if not needed use -1
     * @throws CoreException
     */
    public void createMarker(IResource resource, String message, int lineNumber, String markerType, int severity, int charStart, int charEnd) throws CoreException {
        if (lineNumber <= 0) {
            lineNumber = 1;
        }

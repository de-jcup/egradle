...
public abstract class AbstractGroovyModelBuilder implements ModelBuilder {
    ...
    /**
     * Set a post creation filter - is used when adding created items to model. If
     * it filters the created item, the item (and all children) will not be listed
     * inside model. At postCreationFilter time the items have already the correct
     * gradle item type
     * 
     ...
     * triggering... Only next sibling is resolved and tried to build as item! So
     * build method should not handle to next sibling parts!
     * 
     * @param context
     * @param parent
     * @param current

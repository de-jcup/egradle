...
import de.jcup.egradle.core.model.groovyantlr.AbstractGroovyModelBuilder.Context;

public abstract class AbstractGroovyModelBuilderSupport {
    ...
    public void appendModifiers(Item item, AST modifiers) throws ModelBuilderException {
        if (modifiers == null) {
            return;
        }
    ...
    /**
     * Returns next ast to inspect for further details. If the next hierarchy part
     * is a closure the closure element (CLOSABLE_BLOCK=50) must be returned!
     * 
     * @param enameString
     * @param item
     * @param nextAST
     * @return
     */
    protected ASTResultInfo handleTaskNameResolving(String enameString, Item item, AST nextAST) {
        if (nextAST == null) {
            return null;
        }
    ...
    /**
     * Creates an item, filled with information by context. Will do initial setup
     * for item (e.g. offset, length etc.)
     * 
     * @param context
     * @param ast
     * @return item, never <code>null</code>
     */
    public Item createItem(Context context, AST ast) {
        Item item = new Item();
     ...
     * @return name
     */
    public String resolveMethodCallName(AST methodCall) {
    ...
    private String resolveAsParameterDescription(AST ast) {
        ...
        /*
         * unknown, so return object : name - so after build of model maybe by variable
         * reference check its possible to determine the correct type (in future)
         */

...
/**
 * Full override of SourceBuffer -reason: SourceBuffer does not support offset
 * calculation (which is necessary for eclipse) and also handles not different
...
class ExtendedSourceBuffer extends SourceBuffer {
	...
	public ExtendedSourceBuffer() {
		// lines.add(new StringBuffer()); // dummy row for position [0] in the
		// List
		synchronized (lines) {
			startNewLine();
		}
	 ...
	 * 
	 * @param start
	 *            (inclusive line/ inclusive column)
	 * @param end
	...
	public void write(int c) {
		synchronized (lines) {
			...
			if (crBefore && c != '\n') {
				/*
				 * not a windows new line (\ but a mac new line - so we must
				 * start the new line
				 */
				startNewLine();
			}
	...
	/**
	 * Its necessary to have line endings on each line. If last line has none,

...
public class GradleLanguageElementEstimater {
     ...
     * 
     * @param item
     * @param fileType must be set, if <code>null</code> returned value is always
     *                 <code>null</code>!
    ...
    public EstimationResult estimate(Item item, GradleFileType fileType) {
        ...
        while (pathIterator.hasNext()) {
            ...
            if (current.type == null) {
                /*
                 * jar{ manifest {} } - when manifest is missing delegation target so current
                 * type is null! so just do a break except when element is also not null
                 */

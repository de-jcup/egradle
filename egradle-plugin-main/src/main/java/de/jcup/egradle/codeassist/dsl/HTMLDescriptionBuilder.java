...
public class HTMLDescriptionBuilder {
	 ...
	 * @param element
	 * @param prefix
	 *            can be <code>null</code>, otherwise its rendered before first
	 *            body element
	 * @return description, never <code>null</code>
	 */
	public String buildHTMLDescription(String fgColor, String bgColor, String commentColor,
			LanguageElementMetaData data, LanguageElement element, String prefix) {
	...
	private void appendAppendix(LanguageElement element, StringBuilder description) {
		...
		Type type = (Type) element;
		description
				.append("Now follows a list of <a href='#appendix_methods'>methods</a> and <a href='#appendix_properties'>properties</a> of "
						+ type.getName());
	...
	private void appendLinkToGradleOriginDoc(LanguageElement element, StringBuilder descSb) {
		...
		if (type.isDocumented()) {
			linkToGradleOriginDSLDoc = "https://docs.gradle.org/" + type.getVersion() + "/dsl/" + typeName + ".html";
			if (child != null) {
		...
		String linkToGradleOriginAPIDoc = "https://docs.gradle.org/" + type.getVersion() + "/javadoc/"
				+ typeName.replaceAll("\\.", "/") + ".html";
		if (child != null) {
	...
	private String createHTMLBody(String fgColor, String bgColor, String commentColor, String title,
			StringBuilder descSb) {

		String style = createStyles(fgColor, bgColor, commentColor);

		return "<html><head><title>" + title + "</title><style>" + style + "</style></head><body>" + descSb
				+ "</body></html>";
	}
	...
	private String createStyles(String fgColor, String bgColor, String commentColor) {
		...
		style.append("}\n");
		style.append(
				"pre {background-color: black;color: #999999;border-collapse:separate;border:solid #999999 2px; border-radius:6px; -moz-border-radius:6px;}\n");
		style.append(
				"table {background-color: black;color: #999999;border-collapse:separate;border:solid #999999 2px; border-radius:6px; -moz-border-radius:6px;}\n");
		...
		style.append(
				".originLinkURL {font-size:x-small;color: #999999;font-family:'Courier New', Courier, monospace}\n");

...
public class IDEUtil {
    ...
    private static UnpersistedMarkerHelper buildScriptProblemMarkerHelper = new UnpersistedMarkerHelper("de.jcup.egradle.script.problem");
     ...
     */
    public static IProject createOrRefreshProject(String projectName, IProgressMonitor monitor, ProjectDescriptionCreator projectDescriptionCreator, String... natureIds) throws CoreException {
        if (monitor == null) {
        ...
        /*
         * the next lines are important: only when we do set description on project
         * again the nature will be created AND configured as wished - necessary to get
         * builder running
         * 
         */
    ...
    /**
     * Imports a project by given description file (.project). If a project already
     * exists in workspace with same name it will be automatically deleted (without
     * content delete)
     * 
    ...
    /**
     * Does output on {@link EGradleSystemConsole} instance - asynchronous inside
     * SWT thread
     * 
     * @param message
     */
    ...
    public static void refreshAllProjectDecorations() {
        getSafeDisplay().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                EGradleProjectDecorator decorator = (EGradleProjectDecorator) manager.getBaseLabelProvider("de.jcup.egradle.eclipse.ide.decorators.EGradleProjectDecorator");
                /* decorate */
    ...
    public static void cleanProjects(ProjectContext context, boolean buildAfterClean, IWorkbenchWindow window, IProgressMonitor monitor) {
        if (monitor == null) {
            monitor = NULL_PROGESS;
        }
        ...
        WorkspaceJob cleanJob = new WorkspaceJob(message) {
            ...
            public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
                ...
                if (buildConfigurations != null) {
                    ...
                    try {
                        ResourcesPlugin.getWorkspace().build(buildConfigurations, IncrementalProjectBuilder.INCREMENTAL_BUILD, true, progress.split(1));
                        return Status.OK_STATUS;
                    } catch (CoreException e) {
    ...
    protected static String createBuildInfoMessage(boolean buildAfterClean, final IBuildConfiguration[] buildConfigurations) {
     ...
     * @throws CoreException thrown if there is a problem from the core builder.
     */
     ...
     * @param folder
     * @throws CoreException
     * @throws IllegalArgumentException when folder is not a directory or is
     *                                  <code>null</code>
     */
    ...
    public static void showValidationErrorsOfConsoleOutput(List<String> consoleOutput) {
        ...
        if (result.hasProblem()) {
            try {
                ...
                if (rootFolder == null) {
                    /*
                     * this problem should not occure, because other gradle actions does check this
                     * normally before. as a fallback simply do nothing
                     */
                ...
                if (!file.exists()) {
                    ...
                    buildScriptProblemMarkerHelper.createErrorMarker(resource, "Build file which prodocues error does not exist:" + file.getAbsolutePath(), 0);
                    return;
                }
                IWorkspace workspace = getWorkspace();
     ...
     * 
     * @param showErrorDialog - if <code>true</code> an error dialog is shown when
     *                        root project is {@link Null}. if <code>false</code> no
    ...
    /**
     * Get the root project folder. If not resolvable an error dialog is shown to
     * user and a {@link IOException} is thrown
     * 
    ...
    /**
     * Creates or recreates virtual project - this is done asynchronous. If there
     * exists already a virtual root project it will be deleted full before the
     * asynchronous creation process starts!
     * 
     ...
     * asynchronous creation process starts!
     * 
     * @param postProcessing - a runnable which will be executed after virtual root
     *                       project is (sucessfully) created, can be
     *                       <code>null</code>
     * 
     * @throws VirtualRootProjectException
     */
    public static void createOrRecreateVirtualRootProject(Runnable postProcessing) throws VirtualRootProjectException {
        ...
        Job job = new Job("Virtual root project") {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                ...
                try {
                    Object virtualRootProject = virtualProjectCreator.createOrUpdate(rootProject, partCreator);

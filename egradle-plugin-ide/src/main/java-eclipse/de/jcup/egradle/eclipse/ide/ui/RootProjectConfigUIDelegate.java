...
public class RootProjectConfigUIDelegate implements RootProjectValidationObserver, IPropertyChangeListener {
     ...
     * abstractions of the common GUI blocks needed to manipulate various types of
     * preferences. Each field editor knows how to save and restore itself.
     * 
     * @param parent parent composite to add to
     */
    ...
    private void createValidationGroup(Composite parent) {
        ...
        validationButton.addSelectionListener(new SelectionAdapter() {
            ...
            private void startValidation(OutputHandler validationOutputHandler, MutableGradleConfiguration configuration) {
                try {
                    ...
                    /*
                     * use own progress monitor dialog here - progress service did not work well
                     * because this is in model state
                     */
    ...
    private void createGradleCallTypeGroup(Composite parent) {
		...
		/* @formatter:on */
        gradleCallTypeRadioButton = new ChangeableComboFieldEditor(P_GRADLE_CALL_TYPE.getId(), "Call type", entryNamesAndValues, gradleCallGroup);
        ...
        if (debug) {
            gradleCallTypeRadioButton.getComboBoxControl(gradleCallGroup).setBackground(getColorManager().getColor(new RGB(0, 0, 255)));
        }
		...
		/* @formatter:on */
        shellFieldEditor = new ChangeableComboFieldEditor(P_GRADLE_SHELL.getId(), "Shell", shellTypeComboValues, gradleCallGroup);
        ...
        gradleInstallBinDirectoryFieldEditor = new DirectoryFieldEditor(P_GRADLE_INSTALL_BIN_FOLDER.getId(), "Gradle bin folder:", gradleCallGroup);
    ...
    private void createDefaults(Composite parent) {
        ...
        rootPathDirectoryEditor = new DirectoryFieldEditor(P_ROOTPROJECT_PATH.getId(), "&Gradle root project path", defaultGroup);
        ...
        /* java home default */
        defaultJavaHomeDirectoryEditor = new DirectoryFieldEditor(P_JAVA_HOME_PATH.getId(), "&JAVA HOME (optional)", defaultGroup);
        ...
        defaultJavaHomeDirectoryEditor.getLabelControl(defaultGroup).setToolTipText(defaultJavaHomeDirectoryTooltipText);
    ...
    public void updateCallTypeFields(Object value) {
        ...
        if (EGradleCallType.CUSTOM.equals(callType)) {
            /*
             * do nothing, just keep the editor fields from former default settings - user
             * can change...
             */
        } else {

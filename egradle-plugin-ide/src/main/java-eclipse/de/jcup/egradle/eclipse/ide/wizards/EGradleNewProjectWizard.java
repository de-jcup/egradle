...
public class EGradleNewProjectWizard extends Wizard implements INewWizard {
    ...
    private void createProject(File targetFolder, FileStructureTemplate template, FileStructureTemplate gradleWrapperTemplate, Map<Object, Object> additionalProperties) throws IOException {
        if (context.isGradleWrapperSupportedAndEnabled()) {
            if (gradleWrapperTemplate == null) {
                throw new IllegalStateException("Not able to provide template - gradle wrapper necessary, but not available from IDE");
            }
        }
    ...
    private void importCreatedProject(File targetFolder) throws CoreException {
        ...
        IWizardDescriptor descriptor = PlatformUI.getWorkbench().getImportWizardRegistry().findWizard(EGradleRootProjectImportWizard.ID);
        if (descriptor == null) {

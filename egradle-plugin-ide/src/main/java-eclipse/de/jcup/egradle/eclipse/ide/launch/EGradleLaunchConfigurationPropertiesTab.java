                ...
                tableViewer.setComparator(new ViewerComparator() {
                    public int compare(Viewer iviewer, Object e1, Object e2) {
                        if (e1 == null) {
                            ...
                            return ((PropertyVariable) e1).getName().compareToIgnoreCase(((PropertyVariable) e2).getName());
                        }
                    }
                });
            }
    ...
    protected boolean addVariable(PropertyVariable variable) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (existingVariable.getName().equals(name)) {
                boolean overWrite = MessageDialog.openQuestion(getShell(), "Overwrite variable?", MessageFormat.format("A variable named {0} already exists. Overwrite?", new Object[] { name })); //
                if (!overWrite) {
                    return false;
                }
